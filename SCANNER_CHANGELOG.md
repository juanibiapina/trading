...
- **No-fillable-book skip tracker:** morning eval tallies `tradable=true` candidates that cleared the momentum gates but were skipped as illiquid per the AH-liquidity sanity check (`ask $0.00 x0`, or a wide/thin offer far above the traded price, despite real SIP volume). Records float, catalyst grade, qualified AH price, and the PM outcome (fade vs run). Distinct from the stale-book execution-block (there the name was a fillable detected winner blocked by a stale quote) and the broker-block (`tradable=false`). Standing count 2, both faded (OFAL, BIVI Aug 12→13 — skip validated, 0 cost). Hypothesis: the illiquid skip protects capital; if skipped names instead run in PM on a fillable book, route to the daily email for a marketable-limit reconsideration. Not counted against the detection baseline or selection rate.
- **Stale-book execution-block tracker:** morning eval separately tallies `tradable=true` candidates that cleared all scanner and entry gates but were not entered solely because Alpaca's quote stayed stale through the final scan. Each case records quote age, SIP liquidity, qualified price, PM peak, and hypothetical P&L. Standing count 2, both profitable detected winners (NUWE +82.4%, KUST +45.9%). This measures the cost of the execution-data gap without weakening the live-book safety gate; the repeated feed issue routes to the daily email. Standing count now 3 (NUWE +82.4%, KUST +45.9%, CLRO +34.3% — all detected winners, all profitable hypotheticals).
...
- **Price-floor exclusion tracker:** each morning eval uses the independent whole-market PM sweep (no price/mcap cap) to find real, in-window, volume-backed AH movers that no scan surfaced solely because they sit below `MIN_PRICE = $0.50`, and logs each with its AH signal, PM peak, hypo AH→PM P&L, Alpaca `tradable`, and a **holdable vs uninvestable** verdict (book spread relative to price; gain sustained ≥2 consecutive 5-min bars). Counted as a detection miss (the signal was real and in-window; our universe filter chose not to query it), but `MIN_PRICE` is **not** changed on this evidence — threshold to propose a floor change or a log-only sub-$0.50 pass is ≥3 sub-$0.50 names on ≥3 separate nights that are both >100% and holdable on tight spreads; until then it is a question for the daily email. Standing count 3, **0 holdable** (CISS +164% ~40% spread and unwound in 25 min, OMH `tradable=false`, GOSS flat AH→PM — all 2026-07-28)
...
### 2026-08-13 — Wire Up the No-Fillable-Book Skip Tracker

**Context:** The AH-liquidity sanity check in `post-market-scan.md` (added Jun 26 after TII) instructs the agent to skip a candidate as "illiquid (no AH book)" when the Alpaca book shows `ask $0.00 x0` (or a wide/thin offer far above the traded price) despite real SIP volume, and to **"record it as qualified-but-no-fillable-book for the morning-eval tally."** But no such tally was ever defined in `morning-evaluation.md` — only the broker-block (`tradable=false`) and stale-book (stale quote) execution trackers exist. The instruction pointed at a tracker that did not exist. Result: no-fillable-book skips scatter into individual scan notes and are never aggregated or validated. On Aug 12→13, OFAL and BIVI both cleared the momentum gates (2-AH-scan >10%, extension under ceiling, real accumulating SIP volume) but were skipped on `ask $0.00 x0` / wide-offer books; the scan log explicitly wrote "recorded for the morning-eval tally," yet the 08-13 morning eval reported no running count for them (only a one-line mention in Notes). Earlier no-fillable-book skips (INLF, BJDX, VRM on Aug 5) were similarly stranded. Without aggregation we cannot answer the one question that matters for this skip: does skipping illiquid AH books protect us (they fade) or cost us winners (they run)?

**Evaluation of previous changes:**
- 2026-08-12 two-sided AH-peak-vs-PM-peak tracker: **Held.** The 08-13 morning eval ran the check and scored GXAI's AH peak (+77.5%) vs PM peak (+72%) explicitly noting it was "below the ~+130% extreme zone," so it did not advance the extreme-runner tally (nothing cleared +130%). The two-sided seed (3 fade / 1 continue) was used as intended, not re-derived, and no partial-profit rule was proposed. Correct application on the first cycle.
- 2026-08-11 defer PM-only-gapper count to `pm-open-scan.csv`: **Held.** The 08-13 eval cited "Holdable PM-only-gapper count in the csv = 21" and routed the cluster to Juan's email as the Initiative-6 decision, instead of restating a stale hand-count. Single source of truth held for a second cycle.

**Changes:**
1. **prompts/morning-evaluation.md** — Added a "No-fillable-book skip tracking" paragraph next to the broker-block and stale-book execution trackers. It records date, ticker, float, catalyst grade, qualified AH price, and the PM outcome (fade vs run) for each candidate skipped as illiquid per the AH-liquidity sanity check. Seeded with OFAL and BIVI (Aug 12→13, both faded in PM = skip correct, standing count 2, 0 cost). Explicitly distinct from the stale-book and broker-block trackers, and not counted against the detection baseline or selection rate.
   - Why: The scan prompt has instructed recording these into a "morning-eval tally" for ~7 weeks, but the tally never existed, so the data was lost. This is a recurring pattern (OFAL, BIVI, INLF, BJDX, VRM across recent nights) that needs aggregation to be validated.
   - Hypothesis: The next candidate skipped on the AH-liquidity sanity check is logged against the 2-case seed with its PM outcome and the count is re-stated. After ~5–10 cases we can measure whether the illiquid AH skip is net-protective (skipped names fade) or net-costly (they run on a fillable PM book), and route a marketable-limit reconsideration to the daily email only if the run rate is material. Measurable: (1) the next no-fillable-book skip appears in the morning-eval tally with a fade/run verdict, (2) the standing count advances from 2 without manual re-derivation.

**Updated process:** Added the no-fillable-book skip tracker to the Current Process section (standing count 2, both faded). No scanner thresholds, entry gates, or strategy rules changed.

### 2026-08-12 — Make the AH-Peak-vs-PM-Peak Fade Tracker Two-Sided (Add the Continue Negative Control)

**Context:** The Aug 11→12 night was a clean detect→select→run winner: BAOS entered at $0.90 in AH (2-AH-scan gate, Grade None ultra-low 1.0M float) and exited $2.24 (+148.9%), a rare +149% overnight runner; BOXL +45.4% alongside it. Detection baseline stands at 85.0% (MET). No scanner threshold gap tonight. But the win exposed a **measurement bias** in the morning-eval AH-peak-vs-PM-peak fade tracker. That tracker tests "extreme AH runners (>~+130% from close) peak in after-hours and fade into PM," and it records **only confirming cases** — extreme runners that faded (MSW +292%, DSY +134%, AQB +114%). BAOS is a clean *counterexample*: it topped +177% in AH ($1.97, 19:40 ET) — well past the ~+130% threshold — and *continued* to +208% PM ($2.19). The AH was NOT the better exit. The 08-11 morning eval noted BAOS "extended in PM above its AH peak (continuation)" but did not enter it against the fade tracker, because the tracker only has a fade bucket. A one-sided tracker that logs faders and drops continuers will always trend toward "hypothesis confirmed," and could eventually justify a partial-profit-in-late-AH rule on extreme runners that the full evidence doesn't support.

**Evaluation of previous changes:**
- 2026-08-11 defer PM-only-gapper count to `pm-open-scan.csv`: **Helped.** The morning eval that ran this morning (08-12, evaluating the 08-11 night) cited "`log/pm-open-scan.csv` holdable count stands at 20" and did **not** restate the stale "1 of 6 holdable" hand-count. Hypothesis confirmed exactly: the eval read the CSV-derived count, changed no scanner parameter or entry gate, and left the holdable cluster routed to Juan's email. Single source of truth held.
- 2026-08-07 AH-peak-vs-PM-peak ~+130% seed (DSY/AQB): **Helped but incomplete.** The seed was used (08-11 eval correctly scored DRMA as outside the extreme cluster at +62% AH), but it captured only the fade side. This change completes it by adding the negative control.

**Changes:**
1. **prompts/morning-evaluation.md** — AH-peak-vs-PM-peak check: added an explicit instruction to **record the negative control** (extreme AH runners >~+130% from close that *continued* into PM, PM peak above AH peak), seeded it with BAOS (Aug 11→12, +177% AH → +208% PM continue), and changed the tally to two-sided (faded-vs-continued). Standing seed: 3 fade / 1 continue.
   - Why: the tracker recorded only faders, biasing the extreme-runner-fade hypothesis toward false confirmation; today produced a clean extreme-runner-that-continued the one-sided tracker would have silently dropped. Precedent: the fade-rule tracker already records the correctly-skipped higher-float faders as a negative control.
   - Hypothesis: the next morning eval with a >~+130% AH runner classifies it into the correct bucket (fade or continue) and reports the two-sided standing count, rather than logging only fades. Measurable: within the next extreme-runner nights, the tracker shows a continue count beside the fade count, and no partial-profit rule is proposed unless faders keep outnumbering continuers on SIP-verified peaks.

**Updated process:** Morning-eval AH-peak-vs-PM-peak fade tracker is now two-sided (fade bucket + continue bucket), standing 3 fade (MSW, DSY, AQB) / 1 continue (BAOS). No scanner thresholds, entry gates, or strategy rules changed.
...
**Evaluation of previous changes:**
- 2026-08-05 fade-rule seed (4/11): **Held.** The Aug 5→6 retrospective began at 4/11, found no new sub-3M re-explosion (FRGT and RECT both stayed dead), and left the count at 4/11 with the exception inactive — matching the seed.
- 2026-08-04 tracker synchronization: **Held.** Feed-lag stayed 4 (CLRO was surfaced by the feed, not a feed-lag miss) and price-floor stayed 5 with 0 holdable, both matching their seeds through this cycle.

**Changes:**
1. **prompts/morning-evaluation.md** — Added CLRO to the stale-book execution-block tracker and advanced its seed from 2 to 3 (NUWE +82.4%, KUST +45.9%, CLRO +34.3%, all detected winners, all profitable). Added the Aug 5→6 PAVS continue to the reverse-split-fade tracker and updated the weeks/months-old bucket from 2/3 to 3/4 non-fade.
   - Why: The next evaluation must start from the actual logged sample, not reconstruct it, so the repeated stale-quote cost stays correctly counted (3 lost winners, already routed to the daily email) and the reverse-split recency read reflects both PAVS outcomes.
   - Hypothesis: I expect the next morning evaluation to report a stale-book count of 3 and a weeks/months-old reverse-split split of 3/4 non-fade without manual re-derivation, keep the live-book safety gate unchanged, and keep routing the stale extended-hours quote problem to the daily email. Measurable: the next report states these counts before any new case and changes no scanner threshold or entry gate.

**Updated process:** Stale-book execution-block standing count is 3; reverse-split weeks/months-old bucket is 3/4 non-fade. No scanner thresholds, entry gates, or strategy rules changed.

### 2026-08-05 - Keep the Fade-Rule Tracker Seed Current

**Context:** The Aug 4→5 evaluation logged BJDX (936K float, Grade None) as a liquid sub-3M SPIKE→FADE re-explosion: it fell from a $1.55 AH peak to $1.18 by the final scan, then reached a $1.73 SIP PM peak. The prompt seed still ended at the prior 3/10 sample, so the next evaluation would undercount the tracker.

**Evaluation of previous changes:**
- 2026-08-04 tracker synchronization: **Helped.** The evaluation used the 3/10 fade seed, added BJDX, and reported 4/11. Feed-lag remained 4 and the price-floor tracker remained 5 with 0 holdable cases, matching their seeds.

**Changes:**
1. **prompts/morning-evaluation.md** - Added BJDX to the SIP-verified sub-3M fade tracker and advanced its seed from 3/10 to 4/11.
   - Why: The next evaluation must start from the complete sample so the inactive fade exception is not assessed against stale arithmetic.
   - Hypothesis: I expect the next sub-3M fade evaluation to begin at 4/11, append its outcome with both AH-first-sighting and PM-open hypothetical returns, and keep the exception inactive unless the full SIP-verified sample reaches at least 80%. Measurable: the next report states 4/11 before any new case and does not present the exception as one case from promotion.

**Updated process:** The sub-3M fade-rule tracker seed is 4/11. No scanner threshold, entry gate, or strategy rule changed.
...
**Evaluation (2026-08-05):** Helped. The Aug 4→5 evaluation started from the 3/10 fade seed, added BJDX as a liquid re-explosion, and reported 4/11 while retaining the inactive ≥80% exception. It also kept the feed-lag tally at 4 and price-floor tally at 5 with 0 holdable cases, as seeded.
...
**Evaluation of previous changes:**
- 2026-07-29 Yahoo prev-close anchoring warning: **Helped.** CAST reproduced the artifact (Yahoo $1.70 prev close vs $1.20 actual regular close); `check-prices.py --pm-history` printed the warning and corrected the peak to +22.5% from the regular close. The morning eval used the $1.20 anchor. AAPL printed no warning.
- 2026-07-28 Price-floor exclusion tracker: **Working, no new case.** The eval re-stated 3 total / 0 holdable and left `MIN_PRICE` at $0.50.
- 2026-07-24 In-window feed-lag tracker: **Working, no new case.** The eval re-stated the standing count of 3 and kept the independent data-source recommendation routed to the daily email.

**Changes:**
1. **prompts/morning-evaluation.md** — Added the Jul 29→30 CRE cycle as a separate negative case, advanced the SIP-verified sub-3M tally from 3/6 to 3/7, and removed the false statement that one more positive case could reach a 4/5 threshold. The Current Process summary now carries the same 3/7 count.
   - Why: a stale seed would omit the newest result, while the bad arithmetic made a weakening hypothesis look one case from promotion.
   - Hypothesis: the next morning eval scores sub-3M faders against the seven-case seed, reports the full-sample rate, and does not route or apply the PM-open exception while the overall SIP-verified rate remains below 80%. Measurable: the next fade case starts from 3/7, not 3/6, and no note says one additional winner alone reaches the trigger.
2. **scripts/scan.py** — Fixed percent escaping in the `--day-movers` help text so `python3 scripts/scan.py --help` completes instead of raising `ValueError: unsupported format character 'm'`.
   - Why: today's required verification exposed a broken CLI help path. The scan logic itself was unaffected.
...
**Context:** Jul 28→29 was a solid night — AMIX (Grade None) entered @ $4.02, PM continued to SIP $5.29 (+31.6% on entry) and sold @ $4.68 (+16.4%); ONMD (Grade B) entered and held. No detection or selection defect. The one recurring, in-scope data-quality issue surfaced again in the 07-28 morning eval: the **Yahoo prev-close anchoring artifact**. EGG showed a "+170% PM" headline that was a Yahoo `previousClose` of $2.05 (two closes ago); EGG had already run +96% during the 07-28 regular session (real regular close ~$4.02), so its true AH→PM continuation was only AH +23% → PM +40% from the regular close. The eval had to detect this and hand-correct the winner determination, and the note itself is tagged "recurring." `get_ah_history` already fetched `regularMarketPrice` (the actual most recent regular close) but never used it; `get_pm_history` didn't fetch it. So the tool prints the inflated anchored change every time, and each morning eval must catch the artifact by hand.

**Evaluation of previous changes:**
- 2026-07-28 Price-floor exclusion tracker (CISS/OMH/GOSS): **Working — validated.** The 07-28 morning eval ran the tracker as designed — the whole-market PM sweep surfaced no new sub-$0.50 in-window AH movers, so it re-stated the standing count (3, 0 holdable) UNCHANGED and made no `MIN_PRICE` change. Both criteria met: count re-stated not re-derived; threshold untouched.
- 2026-07-28 Sub-3M fade seed current (WLDS, 3 of 6): **Working — validated.** The 07-28 eval scored the night against the current 6-case seed, cited "standing 3 of 6," correctly logged EGG as an 8.3M-float higher-float negative control (not counted toward the ≥4/5 sub-3M trigger), and made no entry-rule change. Seed was current, not re-derived stale.
- 2026-07-24 In-window feed-lag miss tracker (standing 3, escalation reached): **Working — no regression.** The 07-28 eval re-stated the standing count (3), found no new case (AMIX caught in real time), and kept the whole-universe AH cross-check recommendation routed to the daily email.
- 2026-07-03 Quote-freshness guard on P&L: **Working — validated across pulses.** The 07-28 morning eval and the 07-29 10:30 position eval both flagged stale Alpaca 20:00Z quotes (AMIX/ONMD) and priced P&L off live SIP PM bars instead. No stale figure printed as a real P&L.

**Changes:**
1. **scripts/check-prices.py** — Added `regular_close` (Yahoo `regularMarketPrice`) to the `get_pm_history` return (it was already captured in `get_ah_history` but unused). In both the `--pm-history` and `--ah-history` print blocks, when the anchoring `previous_close` diverges from the actual last regular close by more than 15%, print an `ANCHOR WARNING` line that names both closes and reports the AH/PM peak change measured from the **regular close** (the tradeable-continuation number), not the stale prev-close.
   - Why: the prev-close anchoring artifact is recurring (EGG this cycle) and directly distorts the morning eval's winner determination — a +40%-from-regular-close bounce reads as "+170%." The correct anchor (`regularMarketPrice`) was already in the payload for AH and one field away for PM; surfacing it costs nothing and removes a hand-correction the eval performs every time a name ran hard the prior regular session. The 15% gate keeps the warning silent on normal names (verified: AAPL prints no warning).
   - Hypothesis: the next time a candidate ran hard during the prior regular session (prev-close diverges >15% from regular close), `check-prices.py --pm-history`/`--ah-history` prints the ANCHOR WARNING with the corrected change, and the morning eval reads the regular-close-anchored number without hand-deriving it. Measurable: (1) the next high-Day% name shows the warning line and the regular-close change; (2) normal names (prev-close ≈ regular close) print no warning and are unchanged. If a future EGG-type name is again hand-corrected in the eval with no tool warning, the change didn't take.

**Updated process:** `check-prices.py` auto-flags the Yahoo prev-close anchoring artifact and reports AH/PM change from the actual regular close when they diverge >15% (tooling/output refinement, no gate/threshold change). (No scanner parameters changed.)

**Evaluation (2026-07-30):** Helped. CAST reproduced the same two-close artifact: Yahoo prev close $1.70 versus actual regular close $1.20. The tool printed `ANCHOR WARNING` and corrected the PM peak to +22.5% from the regular close; the morning eval treated CAST as only a small move from $1.20. AAPL remained unchanged with no warning, satisfying both measurable checks.

### 2026-07-28 — Codify the Price-Floor Exclusion Tracker (CISS/OMH/GOSS) + Keep the Sub-3M Fade Seed Current (WLDS Negative Case)

**Context:** Jul 27→28 was a clean, full-coverage night (7/7 scheduled + 2 bonus = 9 scans). **INLF** was detected at 22:30 CET, confirmed across 5 more AH scans, entered @ $4.12 on the 2-AH-scan gate, and PM continued to a SIP peak $5.20 (+58.5% from close) — sold @ $4.89 for **+18.7%**. The AH-build→PM-continuation pattern worked end to end. Both defensive rules were vindicated by PM follow-through (the +150% ceiling skipped BIYA, which then faded −29.5% off its AH peak; the SPIKE→FADE skip on WLDS, whose PM peak came in below both its AH peak and the prior close). The one genuinely new finding: **three of the five biggest PM movers were sub-$0.50 and structurally invisible to the scanner** — CISS (+164% PM, real +37% AH signal at 17:15 ET on 2.95M sh/2,611 trades, in-window), OMH (+65%), GOSS (+45%). All had real, volume-backed, in-window AH footprints; none was ever queried because `scan.py` enforces `MIN_PRICE = $0.50`. That is a **new miss category** (the scanner deliberately not looking) distinct from feed-lag, PM-only gappers and true-tail surges. The morning eval opened the tally ad hoc in Notes and routed the question to the daily email, but there was **no standing tracker** — the same gap the 2026-07-24 in-window feed-lag entry closed.

**Evaluation of previous changes:**
- 2026-07-24 In-window feed-lag miss tracker: **Working — validated.** Today's eval ran the tracker as designed, re-stated the standing count (**UNCHANGED at 3**: BTCT, KUST, WLDS), found no new case, and kept the reached ≥3 escalation (whole-universe AH-data-source cross-check) on the daily email. It also logged a partial BIYA feed discrepancy (SIP $5.62 vs scan $4.79 at 18:00 ET, caught up by 18:30 ET) as a data-quality observation rather than inflating the count. Both criteria met: count re-stated not re-derived; no scanner parameter changed on account of it.
- 2026-07-24 Reverse-split recency seed current (PAVS): **Working — no regression.** Today's eval scored the night against the 7-case seed, correctly declined to add WLDS (5-week-old 1-for-3 split, but a skip-track name with a stale partnership catalyst, not a split-squeeze entry), and re-stated the recency split unchanged (this-week 3/3 fade; weeks/months-old 2/3 non-fade). The seed was current, so nothing was re-derived stale.
- 2026-07-21 Baseline-gap check: **Working — still holding.** The eval surfaced the carried 07-17/07-20 gap and correctly found **no new gap** (07-24 carries a baseline line), then incremented for the evaluated session only (54→55).
- 2026-07-17 Render missing float as `n/a`: **Working — no regression.** All candidates carried real float values; nothing to annotate.
- 2026-07-14 Broker-block early gate + tally: **Insufficient data (not exercised).** Both entries (INLF, BZFD) filled normally. OMH was `tradable=false` but never qualified, so it is correctly not a broker-block row. Tally unchanged.
- 2026-07-03 Quote-freshness guard on P&L: **Working — validated across pulses.** The morning eval verified all three Alpaca marks against live SIP/Yahoo PM (INLF, BZFD, DOMO all confirmed live, not stale); the 10:30 position eval found `broker.js quote` returning the prior 20:00Z close for all three and priced every exit off SIP 5Min bars instead. No stale figure printed as a real P&L.

**Changes:**
1. **prompts/morning-evaluation.md** — Added a **price-floor exclusion tracker** after the in-window feed-lag tracker: each morning, use the independent whole-market PM sweep (no price/mcap cap) to find real, in-window, volume-backed AH movers that no scan surfaced solely because they sit below `MIN_PRICE = $0.50`, and log each with close, AH signal (SIP sh/trades), PM peak, hypo AH→PM P&L, Alpaca `tradable`, and a **holdable vs uninvestable** verdict (book spread relative to price + whether the gain held ≥2 consecutive 5-min bars). Seeded with CISS, OMH, GOSS; standing count 3 (all one night), **0 holdable**. Explicit instruction **not** to change `MIN_PRICE` here; threshold to propose a floor change or a log-only sub-$0.50 pass is ≥3 sub-$0.50 names on ≥3 separate nights that are both >100% **and** holdable on tight spreads — until then it routes to the daily email as a question.
   - Why: the price floor is a new, previously untracked miss category, and one night's evidence (a ~40% spread, a name not even tradeable, a flat third) argues the blind spot is cheap. Without a standing tracker the count would be re-derived by hand each morning and the decisive metric (how many are *holdable*) would stay invisible — the same failure the feed-lag and PM-only-gapper trackers fixed.
   - Hypothesis: the next sub-$0.50 mover is logged against the 3-case seed with its investability verdict and the standing count (total + holdable) is re-stated, and no `MIN_PRICE` change is made until the ≥3-nights-holdable threshold is met. Measurable: (1) the next price-floor exclusion is appended with a holdable/uninvestable label and the count re-stated; (2) `MIN_PRICE` stays 0.5 (measurement + escalation only). If a future sub-$0.50 mover is again counted ad hoc with no running holdable count, the change didn't take.
2. **prompts/morning-evaluation.md** — Brought the fade-rule sub-3M-float seed current: added **WLDS** (Jul 27→28, float 2.0M, AH SIP peak $4.35 @16:10 ET faded to $3.16, PM peak only $3.10 on 31K sh/345 trades — below both its AH peak and the prior close) as a **fell-short** case, moving the standing count from **3 of 5 to 3 of 6** and noting the ≥4/5 trigger has moved further away, with the hypothesis explicitly marked as weakening (two consecutive cycles have added negative cases).
   - Why: today's log already carries WLDS as a sub-3M negative control but the prompt seed still read "3 of 5." A stale seed on a *weakening* hypothesis is the dangerous direction — it makes the live-flip trigger look nearer than the evidence supports. The live-flip handoff stays withdrawn.
   - Hypothesis: the next sub-3M fader is scored against the current 6-case seed and both hypos (a) AH-first-sighting and (b) PM-open re-entry are recorded; the exception is not wired into `Day Trading.md` while the count sits at 3/6. Measurable: (1) the next sub-3M fader is logged against the 6-case seed; (2) no entry-rule change (data collection only). If a future eval cites "3 of 5," the change didn't take.

**Updated process:** Morning eval now runs a price-floor exclusion tracker (standing count 3, 0 holdable, no `MIN_PRICE` change — escalation to email only); fade-rule sub-3M seed current at 3 of 6 with the hypothesis marked weakening (measurement/seed-currency refinements, no gate/threshold change). (No scanner parameters changed.)

### 2026-07-24 — Codify the In-Window Feed-Lag Miss Tracker (WLDS, ≥3 Trigger Reached) + Keep the Reverse-Split Recency Seed Current (First Non-This-Week Fade, PAVS)

**Context:** Jul 23→24 was a clean, full-coverage night (7/7 scheduled + 2 bonus = 9). The winner **LVWR** (Grade A, Q2 revenue +55%, float 10.2M) ignited at **19:15 ET — 45 min after the 00:30 final scan** and ran to a SIP PM peak $2.09 (+171%): a **late-AH-tail true-tail** surge, correctly excluded from the baseline (detection holds 45/50, 90.0%, BASELINE MET). No scanner threshold defect on the winner. But today's own morning eval surfaced a genuine **in-window detection failure**: **WLDS** traded a real, heavy-volume AH surge to SIP $3.51 (+57% from close) at **18:20–18:25 ET — inside the scanned window** (between the 00:00 and 00:30 scans) on 746–793K sh/bar / ~6,900 trades, yet it **never appeared in any evening scan**. At 18:30 ET (the 00:30 final scan) SIP showed WLDS +40%, which should have triggered — classic TradingView postmarket **feed-lag**. WLDS had no prior footprint, so the 2026-07-15 final-scan cross-check (pipeline/already-tracked names only) structurally could not catch it. This is the **third** in-window feed-lag miss (BTCT Jun 30, KUST Jul 15, WLDS Jul 24) and the **first clearly enterable and profitable one** (in-window at 18:30 ET, +48% available: AH $2.50 → PM peak $3.69). The eval counted it ad-hoc in Notes and recommended routing an AH-data-source-verification note to the daily email — but there was **no standing tracker**, so the pivotal recurring-failure count was re-derived by hand each morning.

**Evaluation of previous changes:**
- 2026-07-23 Reverse-split recency seed current (LGCL, months-old 2/2): **Working — validated, and already generating the next data point.** Today's eval scored **PAVS** (1-for-100, ~4 weeks old, faded −18.4%) against the updated seed and correctly logged it as the **first non-this-week fade**, moving the months-old bucket to 2/3. The seed did its job (the new reverse-split name was scored against a current seed and its recency bucket recorded); this entry keeps the prompt seed in step with that running tally.
- 2026-07-22 PM-only-gapper seed current (SXTC, holdable count): **Working — validated.** Today's eval ran the tracker as designed: it scored the night's biggest raw mover (LVWR +171% SIP) against the 6-case seed, correctly classified it an **AH→PM continuation (not a PM-only gapper)** because LVWR had an after-hours footprint, and **re-stated the standing holdable count (1 clear, SXTC, of 6) UNCHANGED**. Both measurable criteria met.
- 2026-07-21 Baseline-gap check: **Working — still holding.** Today's eval surfaced the carried gap (07-17, 07-20 recorded no `Days tracked:` line) and correctly found **no new gap** (the preceding trading day 07-23 carries a baseline line), then incremented Days tracked by 1 for the evaluated session only (53→54). No silent drift.
- 2026-07-17 Render missing float as `n/a`: **Working — no regression.** Tonight's candidates carried real float values; no missing-float name to annotate.
- 2026-07-14 Broker-block early gate + tally: **Insufficient data (not exercised).** All 4 entries (PAVS, AKAN, JEM, ANY) filled normally; no `tradable=false` block. Tally unchanged.
- 2026-07-03 Quote-freshness guard on P&L: **Working — validated.** The eval flagged Alpaca's JEM `positions` mark stale ($6.24 / +16.2%, quote stamped 20:00Z prior close) and priced P&L off the live SIP PM level (~$5.6, +2–5%) instead. No stale Alpaca figure printed as a real gain.

**Changes:**
1. **prompts/morning-evaluation.md** — Added an **In-window feed-lag miss tracker** after the late-AH-tail tracker: each morning, if a real AH mover (>10% AH on accumulating SIP volume) traded *during* the scanned window (before 18:30 ET) yet was **never surfaced by any evening scan**, record it as a TradingView feed-lag miss (date, ticker, surge time, SIP level at the 18:30 ET last-scan timestamp, hypo AH-entry → PM-peak P&L). Explicitly distinguished from a *pipeline* name (recoverable by the post-market-scan final-scan cross-check) and from PM-only gappers / true-tail surges (structural blind spots) — this **is** a genuine detection failure, but its cause is data-source reliability, so the fix is an independent AH-data-source cross-check, not a scanner parameter. Seeded with BTCT, KUST, WLDS. **Standing count 3 → escalation trigger REACHED:** route an AH-data-source-verification recommendation (whole-universe independent gainers cross-check at the final scan) to Juan's daily email.
   - Why: the feed-lag miss is now a recurring pattern (≥3), the eval already counted it by hand and recommended the email escalation, but with no standing tracker the count was re-derived each morning and the escalation lived only in one night's Notes. Codifying it makes the count durable and the escalation explicit — mirroring the validated PM-only-gapper / reverse-split seed-maintenance pattern.
   - Hypothesis: the next in-window feed-lag miss is logged against the seed with its SIP evidence and the standing count re-stated, and the ≥3 escalation routes an AH-data-source recommendation to Juan's email (not applied as a live scanner change here). Measurable: (1) the next feed-lag miss is appended with the count re-stated; (2) no scanner parameter/gate changes on account of this edit (measurement + escalation only). If a future in-window feed-lag miss is again surfaced only ad-hoc with no running count, the change didn't take.
2. **prompts/morning-evaluation.md** — Brought the reverse-split-squeeze fade seed current: added **PAVS** (Jul 23→24, 1-for-100, ~4 weeks old, float 857K, Grade C, entered $7.17 → −18.4%, faded) and updated the recency split from "months-old 2/2 non-fade" to **"weeks/months-old 2/3 non-fade (EDHL, LGCL continue; PAVS faded)"**, noting PAVS is the first non-this-week fade (older bucket now mixed; this-week 3/3 fade still clean).
   - Why: today's log tally already carries PAVS as the first non-this-week fade, but the prompt seed had drifted behind it. Keeping the seed current mirrors the validated 07-23/07-22/07-17 seed-maintenance pattern and stops the recency signal from being re-derived stale.
   - Hypothesis: the next reverse-split entry is scored against the current 7-case seed (this-week 3/3 faded vs weeks/months-old 2/3 non-fade), and at the this-week resolve condition (4/4 fade or a this-week continuation) the recency recommendation routes to Juan's email rather than a live gate. Measurable: (1) the next reverse-split name is logged with its recency bucket against the updated seed; (2) no live gate changes (data collection only). If a future reverse-split entry is scored against the stale "2/2" seed, the change didn't take.

**Updated process:** Morning eval now runs an in-window feed-lag miss tracker (standing count 3, escalation-to-email trigger reached); reverse-split fade seed current at 7 cases with the recency split now weeks/months-old 2/3 non-fade (measurement/seed-currency refinements, no gate/threshold change). (No scanner parameters changed.)

### 2026-07-23 — Keep the Reverse-Split Recency Seed Current (Second Months-Old Non-Fade, LGCL)

**Context:** Jul 22→23 was a clean, full-coverage night (7/7 scheduled scans + 2 bonus = 9). The winner **LGCL** (Personnel Services, float 1.6M, Grade None, 40:1 reverse split Oct 2025 = months-old) was **detected across the 00:00 + 00:30 CET AH scans, entered @ $1.58** on the 2-AH-scan gate, then PM continued to a SIP peak **$2.47 (+160% from close, +56% on entry)** — a textbook detect→select→continuation winner, the best outcome of the cycle. Detection baseline 45/50 (90.0%, BASELINE MET). No scanner defect, no threshold gap. The one in-scope, data-backed refinement surfaced by today's own morning eval: LGCL is a new **reverse-split** data point — a **months-old** split (not this-week) that **continued** (+56%), which the eval added to its running tally ("months-old 2/2 non-fade: EDHL, LGCL"). But the **prompt seed was stale**: it listed only EDHL as the months-old non-fade and framed the recency split as "the only non-fade is a months-old split (EDHL)." Bringing the seed current mirrors the validated 2026-07-22 (SXTC) and 2026-07-17 (reverse-split) seed-maintenance fixes and keeps the recency signal from being re-derived stale each morning.

**Evaluation of previous changes:**
- 2026-07-22 PM-only-gapper seed current (SXTC, holdable count): **Working — validated.** Today's eval ran the tracker exactly as designed: it scored the night's biggest raw mover (LGCL +147%) against the current 6-case seed, correctly classified it an **AH→PM continuation (not a PM-only gapper)** because LGCL moved >10% in AH and was detected, and **re-stated the standing holdable count (1 clear, SXTC, of 6) UNCHANGED**. Both measurable criteria met (new mover logged against the updated seed; count re-stated, not re-derived by hand).
- 2026-07-21 Baseline-gap check (morning-evaluation.md): **Working — still holding.** Today's eval surfaced the carried gap ("2026-07-17 and 2026-07-20 recorded no `Days tracked:` line") and correctly found **no new gap** (the immediately-preceding trading day 07-22 carries a baseline line), then incremented Days tracked by 1 for the evaluated session only (52→53). No silent drift, no false gap.
- 2026-07-17 Render missing float as `n/a`: **Working — no regression.** Tonight's candidates all carried real float values (LGCL 1.6M, etc.); no missing-float name to render, so nothing to annotate. Change still holds.
- 2026-07-14 Broker-block early gate + tally: **Insufficient data (not exercised).** All 4 qualified candidates (LGCL, DOMO, AEHL, PLAG) filled normally; no `tradable=false` block this night. Tally unchanged.
- 2026-07-03 Quote-freshness guard on P&L: **Working — validated.** The morning eval flagged Alpaca's LGCL `positions` mark stale ($2.01 / +27.2%) and its `quote` stamped 20:00Z (prior close, bid $0.82/ask $1.16), and priced P&L off the live PM print ($2.35, +48.7%) instead. No stale Alpaca figure printed as a real loss/gain.

**Changes:**
1. **prompts/morning-evaluation.md** — In the reverse-split-squeeze fade tracker's known-cases seed: added **LGCL (Jul 22→23, 40:1, months-old, float 1.6M, Grade None, entered $1.58 → PM peak $2.47 +56%, continue)** and updated the recency-split summary from "the only non-fade is a months-old split (EDHL)" to **"months-old splits 2/2 non-fade (EDHL mild continue, LGCL continue)."**
   - Why: the running tally in today's log already carries LGCL as the second months-old non-fade, but the prompt seed had drifted behind it (still one months-old case). Keeping the seed current mirrors the validated SXTC (07-22) and reverse-split (07-17) seed-maintenance pattern and stops the recency signal from being re-derived stale.
   - Hypothesis: the next reverse-split entry is scored against a current 6-case seed showing the resolved-looking split (this-week 3/3 faded vs months-old 2/2 non-fade), and at the resolve condition (one more this-week fade → 4/4, or a this-week continuation) a recency-based conviction-downgrade recommendation is routed to Juan's daily email rather than applied as a live gate. Measurable: (1) the next reverse-split name is logged with its recency bucket against the updated seed; (2) no live entry/gate changes on account of this edit (data collection only). If a future reverse-split entry is scored against the stale seed (EDHL-only months-old), the change didn't take.

**Updated process:** Reverse-split fade tracker seed is current at 6 cases with the recency split now reading this-week 3/3 faded vs months-old 2/2 non-fade (measurement/seed-currency refinement, no gate/threshold change). (No scanner parameters changed.)

### 2026-07-22 — Keep the PM-Only-Gapper Tracker Seed Current (First Clear HOLDABLE Case, SXTC)

**Context:** Jul 21→22 was a clean, full-coverage night (7/7 scheduled scans, +2 bonus = 9). No entries (correct — every real AH mover faded: AEHL AH peak $1.38 > PM peak $1.14, KUST reclaim faded instantly). Detection baseline holds at 44/49 (89.8%, BASELINE MET). The night's biggest mover, **SXTC** (+222.9% PM peak, float 32.2M), was a **PM-only gapper** — only +16% in AH on a few hundred sh/bar (never a detectable AH signal), then the >100% explosion fired at 04:00 ET PM open on 1.3–1.85M sh/bar (24k–39k trades). Per the PM-only-gapper rule this is a structural blind spot of the AH→PM strategy, **not** a scanner miss and not counted against baseline. The in-scope, data-backed observation: SXTC is the **first clear HOLDABLE PM-only gapper** — the gain sustained across ≥2 consecutive 5-min PM bars on real volume and held above +130% for hours (a genuine liquid exit window), unlike the prior cases (MBRX/CUPR uninvestable, GLXG instant spike). The PM-only-gapper tracker's *entire hypothesis* is "the blind spot only matters if the gappers are *holdable*," yet the prompt's seed list was stale (5 cases, no SXTC) and carried **no standing holdable count** — so the pivotal metric (how many holdable cases have accrued toward a cluster) was re-derived by hand each morning instead of being visible. This mirrors the 2026-07-17 reverse-split seed-currency fix.

**Evaluation of previous changes:**
- 2026-07-21 Baseline-gap check (morning-evaluation.md): **Working — validated.** Today's 07-22 eval ran the check exactly as designed: it surfaced the carried gap ("2026-07-17 and 2026-07-20 morning evals recorded no `Days tracked:` line") and correctly found **no new gap** because the immediately-preceding trading day (07-21) does carry a baseline line — then incremented Days tracked by 1 for the evaluated session only (51→52), no back-fill, no silent drift. Both measurable criteria met (gap surfaced with specific dates; no false gap on the consecutive-day run).
- 2026-07-17 Render missing float as `n/a`: **Working — still holding.** The 07-20 scans printed missing-float names as `n/a` (not `0`); no scan or eval had to annotate "0 = unknown." No regression this cycle (07-22 had no missing-float candidate to render).
- 2026-07-17 Reverse-split seed current: **Insufficient data (not exercised).** No reverse-split entry or winner this cycle (clean no-trade night). Recency tally unchanged (this-week 3/3 faded). Still open.
- 2026-07-14 Broker-block early gate + tally: **Working — validated.** SHPH was `tradable=false` across the AH scans again (07-21); the flow flagged it early, carried it as untradable, and logged one broker-block row with the hypothetical cost (this instance ≈ nil — SHPH PM peaked only +6.3%, would have lost). Early gate + carry-forward + tally all fired as designed.
- 2026-07-03 Quote-freshness guard on P&L: **Insufficient data (not exercised).** Account flat all cycle (no open positions); no Alpaca P&L to guard. Still validated in prior cycles.

**Changes:**
1. **prompts/morning-evaluation.md** — In the PM-only-gapper tracker: added **SXTC (Jul 22, +223%, holdable)** to the known-cases seed with its investability evidence (the +100% move fired at PM open on 1.3–1.85M sh/bar, held above +130% for hours), and added an explicit **standing holdable count: 1 clear (SXTC) of 6 tracked**, with the cluster threshold (≥3–4 holdable) routed to Juan's daily email rather than applied here, plus a standing instruction to append each new gapper with its label and re-state the count.
   - Why: the tracker's hypothesis turns entirely on the *holdable* count, but the seed was stale (no SXTC) and had no running count — the decisive metric was reconstructed by hand each morning. SXTC is the first clear holdable case, so recording it now (with the count made explicit) mirrors the validated reverse-split/sub-3M seed-maintenance pattern and keeps the tracker from being re-derived stale.
   - Hypothesis: the next PM-only gapper is scored against a current 6-case seed and the standing holdable count advances (or holds) explicitly, and only a ≥3–4 holdable cluster routes an early-PM-workflow recommendation to Juan's email. Measurable: (1) the next gapper is logged with its holdable/uninvestable label against the updated seed and the count is re-stated; (2) no live gate/threshold changes on account of this edit (data collection only). If a future gapper is logged without the label or the holdable count is not re-stated, the change didn't take.

**Updated process:** PM-only-gapper tracker seed is current at 6 cases with an explicit standing holdable count (1 clear, SXTC) and a routed-to-email cluster threshold (measurement/seed-currency refinement, no gate/threshold change). (No scanner parameters changed.)

### 2026-07-21 — Codify Baseline-Gap Detection (Stop the Detection-Rate Chain From Silently Drifting on a Skipped Morning Eval)

**Context:** Jul 20→21 was a clean, full-coverage night (7/7 scheduled scans). The winner **HIHO** (Grade B, Q1 FY2027 earnings +29.2% YoY sales, float 2.8M) was **detected across 6 AH scans and entered @ $1.50** on the 2-AH-scan gate, then PM re-exploded to $1.89 (+133% from close, +26% on entry) — a textbook detected-and-traded winner, no scanner defect. Detection baseline 44/49 (89.8%, BASELINE MET). The one concrete, in-scope, data-backed defect surfaced by today's own morning eval: the **baseline tracking chain silently drifted**. The count is derived by grep-tailing the most recent log with a `Days tracked:` line and blindly adding +1. On **07-17 (Fri) and 07-20 (Mon)** the morning evals recorded **no `Days tracked:` line** (07-17 had only position evals; no morning-eval retrospective ran), so today's eval grep-tailed the stale 07-16 baseline, had to reconstruct by hand, and could only guess "+1." Two failure modes: (1) a skipped morning eval means that night's winner is **never diagnosed** — a detection miss could hide with no trace; (2) the blind "+1" ambiguously conflates "one trading day elapsed" with "one day was evaluated," so the denominator's meaning drifts whenever an eval is skipped. The scheduler (why the eval didn't run) is out of scope, but making the chain **self-documenting** — surfacing skipped retrospectives instead of absorbing them silently — is squarely a process/measurement fix.

**Evaluation of previous changes:**
- 2026-07-17 Render missing float as `n/a`: **Working — validated.** The 07-20 evening scans printed **CRNG** and **PAPL** with Float **`n/a`** (empty TradingView field) instead of `0`; no scan or eval had to annotate "0 = unknown," and no candidate was filtered or graded differently for the missing float (float is not a gate). Change took exactly as designed.
- 2026-07-17 Reverse-split fade seed current (this-week 3/3): **Insufficient data (not exercised).** Neither entry (HIHO, PAPL) nor the winner had a reverse-split catalyst; no new tally row. Still open.
- 2026-07-16 Realistic PM-open re-entry hypo on the sub-3M fade tracker: **Insufficient data (not exercised).** No sub-3M AH-fader was *skipped* this night — HIHO (2.8M) faded $1.69→$1.28 then PM re-exploded to $1.89, but we **entered** it (before the fade), so it corroborates the sub-3M re-ramp hypothesis without being a fade-rule false negative. Standing sub-3M count held at 3 of 5 (≥4/5 trigger NOT met). The two-hypo logging had no skipped case to run.
- 2026-07-14 Broker-block early gate + tally: **Working — validated again.** SHPH was `tradable=false` across all AH scans (the recurring low-float pharma broker-block); the flow flagged it early and carried it as untradable without re-running the full workup, and it was logged once to the tradability-gap dataset. The early gate + carry-forward fired as designed.
- 2026-07-03 Quote-freshness guard on P&L: **Working — validated across pulses.** The morning eval and the 10:30 position eval both flagged Alpaca `positions` quotes as stale (HIHO ts 20:59Z / PAPL ts 20:00Z = prior close; PAPL book `ask $0.00 x0`) and priced P&L off live SIP PM bars (HIHO +10.7%, PAPL −2.7%). No stale Alpaca figure printed as a real loss.

**Changes:**
1. **prompts/morning-evaluation.md** — Added a **baseline-gap check** to Step 1: after grep-tailing the latest `Days tracked:` line, confirm that log is the immediately-preceding trading day; if intervening trading-day logs exist with no `Days tracked:` line, list those dates as a **baseline gap** and surface it in Notes (a skipped retrospective = that night's winner never diagnosed, so a detection miss could hide), rather than silently absorbing the gap into a single +1. Reinforced in Step 5: increment "Days tracked" by 1 **for the evaluated session only** — do not back-fill skipped days (unknown detection status must not inflate/deflate the rate) — and record any un-evaluated dates so the smaller sample is visible.
   - Why: the baseline chain relies on each morning eval writing a `Days tracked:` line; when an eval is skipped (07-17, 07-20), the grep-tail picks a stale baseline and the blind "+1" hides both the drift and the un-diagnosed night. Today's eval caught this by hand; codifying it makes the detection each morning rather than model-luck.
   - Hypothesis: the next time a morning eval is skipped, the following eval automatically surfaces the gap (lists the un-evaluated trading dates in Notes) and increments by 1 only for the evaluated session, instead of silently reconstructing a drifted count. Measurable: (1) the next baseline-chain gap is reported with the specific skipped dates; (2) on a normal consecutive-day run, the check finds no gap and the count advances +1 as before (no false gap flag). If a future skipped-eval day passes without the following eval flagging the gap, the change didn't take.

**Updated process:** Morning eval now runs a baseline-gap check (surfaces skipped retrospectives, no silent denominator drift; measurement/data-integrity fix, no gate/threshold change). (No scanner parameters changed.)

### 2026-07-17 — Render Missing Float as `n/a` + Keep the Reverse-Split Recency Seed Current (this-week 3/3 faded)

**Context:** Jul 16→17 was another clean detection+selection night. The winner **CJMB** (Grade B, Alabama State Univ. pharma partnership, float unknown) was detected across the 00:00 + 00:30 CET scans, **entered @ $1.01**, ran to a PM peak $1.58 (+56.4% on entry, +113.5% from close) and is being **held as a winner** into day 2 (position eval 10:30 CET: CJMB +37.6%, HOLD, trail $1.34). The scanner and every gate did their job — no detection miss, no threshold defect (detection baseline was 89.6% / BASELINE MET at the last full morning eval). Two small, in-scope observations from recent nights: (1) CJMB (and ASPD, and other low-float names) print **Float `0`** in the scanner because the TradingView `float_shares_outstanding` field is empty — and every scan/eval since has had to hand-annotate "scanner 0 = unknown, not a filter" (the 00:00/00:30 CJMB gate notes literally say "float unknown (scanner 0, not a filter)"). A literal `0` is ambiguous with a genuinely tiny float; float is **never** a filter in scan.py (confirmed — `r['float']` is only ever passed to the formatter, never to a gate), so rendering missing float honestly as `n/a` costs nothing and removes the recurring misread risk. (2) Last night added **BIYA** (1-for-10 reverse split effective Jul 13 = this-week, Grade None, float 2.7M) to the reverse-split-squeeze fade tracker as another **fade** (PM peak $4.42 fell short of AH peak $5.19, exited −13.8%). The reverse-split tracker's *prompt seed* still listed only 4 cases and had drifted behind the running tally; with BIYA the **this-week reverse-split bucket now stands 3/3 faded** (ELPW, YYGH, BIYA), the only non-fade being a months-old split (EDHL) — the recency signal the tracker was built to test is now visible and one case from resolving.

**Evaluation of previous changes:**
- 2026-07-16 Realistic PM-open re-entry hypo on the sub-3M fade tracker: **Insufficient data (not exercised).** The Jul 16 morning eval logged PMAX (892K) and GLXG (1.7M) as sub-3M spike→fade **negative controls** (both peaked in the first AH minutes, PM < AH peak, did NOT re-explode) — so no sub-3M fader re-exploded and the two-hypo logging had no case to run. Standing sub-3M count held at 4/5. Still open.
- 2026-07-15 Final-scan feed-lag cross-check: **Working — executed, nothing to rescue.** The 00:30 CET final scan ran the codified cross-check ("Final-scan feed-lag cross-check: All tracked names ... appeared in the fresh 18:30 ET scan — no under-reported/omitted names to rescue via SIP"). The step fired as designed on a night with no feed-lag omission.
- 2026-07-14 Broker-block early gate + tally: **Insufficient data (still open).** Both CJMB and BIYA filled (`tradable=true`); no untradable qualifier arose. No carry-forward or tally row exercised.
- 2026-07-10 Reverse-split-squeeze fade tracker: **Working — validated with a 5th case.** The Jul 16 eval added BIYA (this-week 1-for-10, faded) to the tally exactly as designed and framed it as consistent with the recency hypothesis. This entry keeps the prompt seed in step with that running tally.
- 2026-07-08 Verify PM peaks against SIP: **Working — validated.** CJMB's PM peak was reported as **SIP $1.58** with the Yahoo divergence noted ("Yahoo showed $1.48, under-reported — use SIP"). No Yahoo under-reported peak printed as fact.
- 2026-07-03 Quote-freshness guard on P&L: **Working — validated across pulses.** The Jul 16 morning eval flagged BIYA's Alpaca `positions` mark stale ($4.08 / −10.3%) and priced it off live SIP PM bars (≈ −4.6% against $4.34); the Jul 17 position eval independently flagged BIYA's stale quote and used SIP + `current_price`. No stale figure printed as a real loss.

**Changes:**
1. **scripts/scan.py** — Added a `fmt_float(n)` helper that renders a missing float (source value `0`/empty) as **`n/a`** instead of `0`, and routed both Float-column print sites (regular and PM/AH tables) through it. `fmt_number` is unchanged, so volume/market-cap columns (where `0` is a real value) are untouched.
   - Why: the TradingView float field is frequently empty on the low-float names the strategy targets, and a literal `0` is ambiguous with a genuinely tiny float — every recent scan/eval hand-annotated "scanner 0 = unknown, not a filter." Float is never used as a gate in scan.py (verified), so honest `n/a` rendering is zero-risk and removes the recurring misread.
   - Hypothesis: the next scan with a missing-float name shows `n/a` in the Float column and no scan/eval needs to annotate "0 = unknown"; a genuinely reported tiny float still shows its `K`/`M` value. Measurable: (1) the next missing-float candidate renders `n/a`; (2) no candidate is filtered or graded differently (float is not a gate). If a future scan again prints `0` for a missing float, the change didn't take.
2. **prompts/morning-evaluation.md** — Brought the reverse-split-squeeze fade tracker's known-cases seed current: added **BIYA** (Jul 15→16, 1-for-10, this-week, faded) and recorded the standing **recency split (this-week 3/3 faded vs months-old non-fade)**, plus the resolve condition (one more this-week-split fade = 4/4, or a this-week-split continuation, resolves the signal → route a recency-based conviction-downgrade recommendation to the daily email for Juan, not a live gate here).
   - Why: the prompt seed had drifted behind the running tally (still 4 cases; BIYA was added in the Jul 16 log only), and the recency signal the tracker exists to test is now visible (3/3 this-week faded). Keeping the seed current mirrors how the sub-3M seed is maintained and stops the tracker from being re-derived stale each morning.
   - Hypothesis: the next reverse-split entry is scored against a current 5-case seed and the 3/3-this-week-faded pattern, and at 4/4 (or a this-week continuation) the recency recommendation is routed to the daily email rather than applied as a gate. Measurable: (1) the next reverse-split name is logged with its recency bucket against the updated seed; (2) no live entry/gate changes on account of this edit (data collection only). If a future reverse-split entry is scored against the stale 4-case seed, the change didn't take.

**Updated process:** Scanner renders a missing float as `n/a` (was `0`); reverse-split fade tracker seed is current at 5 cases with the recency split (this-week 3/3 faded) and a routed-to-email resolve condition (measurement/output refinements, no gate/threshold change). (No scanner parameters changed.)

### 2026-07-16 — Record the Realistic PM-Open Re-Entry Economics on the Fade-Rule Sub-3M Tracker (Trigger Reached)

**Context:** Jul 15→16 was another clean, full-coverage night (8 scans, 21:30–00:30 CET). Detection baseline 89.4% (42/47, BASELINE MET), selection 48.9%. One real entry — **TGHL** ($1.34, Grade None, 12.7M float, BUILD-and-hold, cleared the 2-AH-scan gate at 23:30) — which peaked in AH ($1.59 @~18:00 ET) and faded into premarket, exited −18.6% by the 10:30 position eval. The scanner and every gate did their job: no threshold defect, no detection miss. The night's winner, **ATPC** (Pharma, float **475K**, no catalyst), was **detected in 5 evening scans** and correctly skipped SPIKE→FADE for the scanned window (faded opening spike $3.08 → choppy $2.7-3.0 base), then its decisive surge fired at **19:20 ET / 01:20 CET (~50 min after the final scan)** to an AH-tail peak $3.87, extending to a PM peak **$4.90** (+88.5% from close). This is a **fade-rule false negative on a sub-3M-float name** — and it is the decisive one: it brings the sub-3M-float fade-false-negative tally to **4 of 5** (LNKS/RPGL/CRE/ATPC blew past, IOTR fell short), **reaching the ≥4/5 trigger** codified on 2026-07-09 to propose a sub-3M-float PM-open re-check exception to the SPIKE→FADE skip. The morning eval flagged the trigger and routed the exception proposal to the daily email (Juan's decision — a live-entry/selection rule, not mine to flip). The in-scope, conservative move is a **measurement refinement**: the tracker records only the *optimistic* AH-first-sighting hypo (+72%), but the *proposed rule* enters at **PM open**, where most of the move is already banked (ATPC PM-open ~$4.00 → peak $4.90 = only **+22%**). Recording the realistic PM-open re-entry economics sharpens the dataset behind the now-pending decision without touching any live entry or strategy.

**Evaluation of previous changes:**
- 2026-07-15 Final-scan feed-lag cross-check: **Working — step executed.** The 00:30 CET final scan ran the codified cross-check ("cross-checked tracked names against SIP. No under-reported real mover being dropped... No rescue needed") — the step is now in the flow and fired as designed. ATPC's surge came *after* the final scan (19:20 ET), so it was a late-AH-tail/fade case, not a feed-lag under-report at 18:30 ET; the cross-check correctly found nothing to rescue.
- 2026-07-14 Broker-block early gate + tally: **Insufficient data (still open).** No `tradable=false` qualifier arose this night; no carry-forward or tally row exercised.
- 2026-07-10 Reverse-split-squeeze fade tracker: **Insufficient data.** Neither entry/winner (TGHL, ATPC) had a reverse-split catalyst; no new row.
- 2026-07-09 Float-conditional fade trigger: **Working — trigger reached exactly as designed.** ATPC (475K) was bucketed sub-3M and advanced the count to **4/5**, hitting the ≥4/5 threshold; higher-float names stayed excluded. The float-conditional trigger fired on the right evidence (a sub-3M fader), validating the 2026-07-09 alignment. This entry builds on it by measuring the exception's real economics before it goes live.
- 2026-07-08 Verify PM peaks against SIP: **Working — validated.** Morning eval reported ATPC AH peak SIP $3.87 (Yahoo under-reported $3.65) and PM peak SIP $4.90 (Yahoo $3.96); SIP peaks used, divergence noted. No Yahoo under-reported peak printed as fact.
- 2026-07-03 Quote-freshness guard on P&L: **Working — validated.** The 10:30 position eval flagged the Alpaca quote as stale ($1.18 @ 20:59Z prior close) and priced TGHL off live SIP bars ($1.33H→$1.08C, vwap $1.07-1.20). No stale figure printed as a real loss.

**Changes:**
1. **prompts/morning-evaluation.md** — On the fade-rule sub-3M-float tracker: (a) advanced the standing count from "3 of 4" to **"4 of 5 — the ≥4/5 trigger is REACHED"** and added ATPC to the known cases; (b) recorded that the PM-open re-check exception is now **proposed to the daily email for Juan's decision** and is *not* applied to live entries until approved; (c) required each sub-3M fader to log **two** hypos — (a) the optimistic AH-first-sighting entry→PM-peak and (b) the realistic **PM-open re-entry (~04:00 ET price) → PM-peak** — because the proposed rule enters at PM open, so (b) is the number that decides whether the exception actually pays after the AH move is banked (ATPC: (a) +72% vs (b) +22%).
   - Why: the trigger reached, so the exception is now a live decision for Juan — but the tracker measured it with the optimistic AH-first-sighting entry, which overstates the payoff. The proposed rule enters at PM open, where much of the move is already gone; without the realistic number the decision would be made on an inflated dataset.
   - Hypothesis: the next sub-3M fader logs both hypos, and across the 4/5 cases the realistic PM-open re-entry P&L reveals whether the exception pays. Measurable: (1) the next SPIKE→FADE sub-3M skip records a PM-open re-entry→PM-peak figure alongside the AH-first-sighting figure; (2) if the PM-open re-entry P&L is consistently thin/negative, the exception is flagged a mirage despite the trigger; if consistently strong, it's confirmed for Juan. If a future sub-3M fader is logged with only the AH-first-sighting hypo, the change didn't take.

**Updated process:** Fade-rule sub-3M tracker now stands at 4/5 (trigger reached, exception proposed to Juan via email, not auto-applied) and records both the optimistic AH-first-sighting hypo and the realistic PM-open re-entry hypo per sub-3M fader (measurement refinement, no live-entry/threshold change). (No scanner parameters changed.)

### 2026-07-15 — Codify the Final-Scan Feed-Lag Cross-Check (Real-Time SIP Rescue for Pipeline Names)

**Context:** Jul 14→15 was a clean, full-coverage night (7/7 scans). Detection baseline 89.1% (41/46, BASELINE MET), selection 50.0%. Three real entries — **VTAK** ($0.72, Grade None, 2.0M float, BUILD→hold → PM re-explode $1.27, +76% available, the one continuation winner), **GCTK** ($0.87, Grade A, AH peaked +106% @18:15 ET → faded PM, AH was the better exit), **XCUR** ($2.40, Grade None, build→fade). The scanner and every gate did their job — no threshold defect. The one recurring, in-scope, explicitly-flagged defect is **TradingView postmarket feed-lag at the final scan**: the night's winner **KUST** was a real **+34% / 812K-sh** mover at the 00:30 CET / 18:30 ET last scan, but the TradingView feed did not surface it — the *same* failure mode that hit **GCTK** the same night (GCTK we *did* rescue via an ad-hoc SIP cross-check and entered at 00:30) and **BTCT** Jun 29→30 (already +67.6% at 18:30 ET while the 00:30 feed showed it flat). The morning-eval retrospective and the late-AH-tail tracker have now recommended "cross-check Yahoo AH / SIP at the last scan" as *the* fix at least three times, but the recommendation lived only in the retrospective — the **scan prompt had no codified real-time step**, so the rescue happened ad-hoc (GCTK) or not at all (KUST/BTCT, caught only next morning). KUST itself was unenterable (single scan above threshold, surge 5 min pre-scan, fails the 2-scan gate), so this costs detection *cleanliness* and risks mis-skipping a pipeline name (GCTK-style), not the KUST entry specifically. Codifying the existing GCTK rescue is the surgical fix.

**Evaluation of previous changes:**
- 2026-07-14 Broker-block early gate + tally: **Insufficient data.** SHPH was flagged `tradable=false` in the 21:30 regular watch but never reappeared in an AH scan, so it never became a qualified-but-untradable broker-block this night — no carry-forward or tally row was exercised. No other untradable qualifier arose. Still open.
- 2026-07-10 Reverse-split-squeeze fade tracker: **Insufficient data.** None of the three entries (VTAK/GCTK/XCUR) had a reverse-split catalyst; the morning eval recorded "no new row." Not exercised.
- 2026-07-09 Float-conditional fade trigger: **Working — negative-control side exercised.** No sub-3M AH-fader re-exploded (VTAK, 2.0M, was *entered* as a BUILD-and-hold, not a skipped fader; TGHL 12.7M faded as predicted). Standing sub-3M count held at 3 of 4, correctly not advanced.
- 2026-07-08 Verify PM peaks against SIP (Yahoo under-reports): **Working — validated.** VTAK's PM peak was reported as **SIP $1.27** (6.13M sh / 27,277 trades, VWAP $1.04) with the Yahoo divergence noted (Yahoo `--pm-history` showed only $0.92); KUST's AH/PM peaks were SIP-sourced ($1.34 AH / $1.95 PM). No Yahoo under-reported peak printed as fact.
- 2026-07-03 Quote-freshness guard on P&L: **Working — validated across pulses.** The morning eval and both position evals flagged Alpaca `quote` as stale (20:00Z close) and reported P&L against live PM/SIP levels (VTAK/XCUR/GCTK/LVLU). No stale Alpaca figure printed as a real loss.
- 2026-07-02 Verification freshness guard (entry flow): **Insufficient data (still open).** No stale-verification bad-print case; entries were SIP-confirmed real at entry. Not exercised.

**Changes:**
1. **prompts/post-market-scan.md** — Added a **final-scan feed-lag cross-check** to Step 4: at the last scheduled scan (00:30 CET / 18:30 ET), cross-check every already-tracked candidate (any name seen in ≥1 prior AH scan near/above threshold, plus the 21:30 regular-session watch) against SIP bars / Yahoo `--ah-history`; if the TradingView scan under-reports a name that SIP shows above the 10% threshold on real accumulating volume, treat the SIP reading as truth and evaluate it for entry (2-AH-scan gate + all other rules still apply) rather than dropping it on the stale TradingView figure. Explicitly scoped to pipeline names (a brand-new omitted name with no prior footprint stays a morning-eval note). Cites GCTK Jul 14 (rescued), KUST Jul 14 (omitted), BTCT Jun 29–30.
   - Why: the morning eval has recommended this exact cross-check ≥3 times but it lived only in the retrospective; the rescue was applied ad-hoc (GCTK) instead of by rule, so a pipeline name the final TradingView feed under-reports could be mis-skipped in real time.
   - Hypothesis: the next time the final TradingView scan under-reports a tracked candidate, the cross-check surfaces its real SIP level at that scan (logged as "TradingView under-reported X, SIP shows +Y%") instead of it being discovered only in the next morning's retrospective. Measurable: (1) the next feed-lag case on a pipeline name is caught and re-evaluated at the final scan; (2) names TradingView reports correctly are unaffected. If a future pipeline feed-lag name is again first surfaced only in the morning retrospective, the change didn't take.

**Updated process:** Final scheduled scan now cross-checks all tracked/watch names against SIP/Yahoo AH and rescues any the TradingView feed under-reports (real-time codification of the GCTK-style rescue; data-source fix, no threshold change). (No scanner parameters changed.)

### 2026-07-14 — Gate Broker-Blocked Names Early + Track the Alpaca Tradability-Gap Cost

**Context:** Jul 13→14 was a clean, full-coverage night (7/7 scans). Detection baseline 88.9% (40/45, BASELINE MET), selection 51.1%. **VMAR** is the model win: ultra-low float (670K), real SIP volume, BUILD across ≥2 AH scans, detected in the 23:30/00:00/00:30 scans and **entered at 00:00 @ $1.57** on the second qualifying scan, then PM continued to $3.35 (**+113%** on entry, sold +41.4% at exit). No scanner defect — the scanner detected and selected the right stock and it ran. The one recurring, in-scope process defect is the **SHPH broker-block**: SHPH was the strongest BUILD of the night (AH +100%, float 596K, VRatio 5.8x, 17k+ trades/bar) but Alpaca reported `tradable=false`, so it never filled — and the entry flow re-ran the full SIP-verification + catalyst-search workup on it in *all 5* entry scans before re-discovering it can't fill. This is not new: SHPH was `tradable=false` across all 5 entry scans on **Jun 25** too (same ticker, same 5×-wasted-workup pattern). Two gaps: (1) the tradability check sat *after* the expensive SIP/catalyst work in the entry flow, so a known-untradable name burned effort every scan; (2) there is no running tally quantifying how much the Alpaca tradability gap costs the selection rate, so the recurring broker-coverage decision has no dataset behind it. The morning eval flagged SHPH as an "infra note for Juan" but with no cumulative measure.

**Evaluation of previous changes:**
- 2026-07-10 Reverse-split-squeeze fade tracker: **Working — validated with a counter-example.** The Jul 13 morning eval applied it to VMAR (06/17 reverse split, **weeks-old**, float 670K, entered $1.57 → PM peak $3.35, **+113%, CONTINUED**) and explicitly framed it as a recency counter-example: a *weeks-old* split ran hard, unlike the fresh (this-week) split faders ELPW/YYGH (both −17%). The tracker is doing exactly its job — building the split-recency dataset, now with a continue data point supporting recency-over-split-itself.
- 2026-07-09 Float-conditional fade trigger: **Insufficient data.** No AH-fader re-exploded in PM on the Jul 13 night (VMAR was a BUILD-and-hold that we entered, not a skipped fader; SHPH was broker-blocked, not fade-skipped). Standing sub-3M count held at 3 of 4. Not exercised.
- 2026-07-08 Verify PM peaks against SIP: **Working.** VMAR's PM peak was reported as **SIP $3.35** (4.56M sh / 37,981 trades, VWAP $2.62) with the Yahoo divergence noted; SHPH AH peak $5.86 was SIP-sourced. No Yahoo under-reported peak printed as fact.
- 2026-07-03 Quote-freshness guard on P&L: **Working — validated across pulses.** Both the morning eval and the 10:30 position eval flagged the Alpaca `quote` as stale (20:59Z / 20:00Z) and reported VMAR/LVLU P&L against live PM prints. No stale Alpaca figure printed as a real loss.
- 2026-07-02 Verification freshness guard (entry flow): **Insufficient data (still open).** No stale-verification bad-print case; VMAR/LVLU/SHPH were all SIP-confirmed real at entry-time. Not exercised.
- 2026-07-01 (c) SIP volume confirmation: **Working.** VMAR sized only after SIP corroboration (1M+ sh/bar, 6.2k trades at the 21:00Z ramp); SHPH's real BUILD was SIP-confirmed (17k+ trades/bar) — the block was broker-side, not signal-side.
- Ceiling + dead-cat + fade gates (Jun lineage): **Working.** SOBR correctly skipped over the +150% ceiling (Total +204%); JEM (Grade D reverse split, fading) skipped; VMAR well under ceiling.

**Changes:**
1. **prompts/post-market-scan.md** — Made the `tradable` check an **early gate** and added a **carry-forward** rule: verify `broker.js tradable SYM` *before* the SIP-volume/catalyst workup on a fresh candidate, and once a name is flagged `tradable=false` in any scan, carry it forward on later scans as "untradable (carried)" without re-running the SIP verification or catalyst search (it can never fill). Cites SHPH blocked 5× on both Jun 25 and Jul 13.
   - Why: a `tradable=false` state is fixed for the session, but the flow re-did the full expensive workup on SHPH every scan (5×/night, two nights). Checking tradability first and carrying it forward removes the wasted effort with zero selection impact (an untradable name was never enterable).
   - Hypothesis: on a night with a recurring `tradable=false` name, the name is fully worked **once** then carried, and later scan sections do not repeat its SIP/catalyst workup. Measurable: (1) the next multi-scan untradable candidate shows one full evaluation then "untradable (carried)" on subsequent scans; (2) it is still recorded once as a broker-block. If a future night re-runs the full SIP/catalyst workup on an already-`tradable=false` name across scans, the change didn't take.
2. **prompts/morning-evaluation.md** — Added a **broker-block tracker**: each candidate that cleared all entry gates but never filled on `tradable=false` gets a running-tally row (date, ticker, scans qualified, catalyst grade, hypothetical AH-entry→PM-peak P&L). Seeded with SHPH (Jun 25) and SHPH (Jul 13, hypo +29% to +70%). Explicitly not counted against detection baseline or selection rate.
   - Why: the broker-block recurs (SHPH twice, both times the cleanest real-volume qualifier of the night) and suppresses the selection rate independent of scanner quality, but there was no cumulative measure — so the recurring broker-coverage decision had no dataset. Quantifying the hypothetical P&L cost per case gives the user a data-backed basis for a broker switch.
   - Hypothesis: the cumulative cost of the Alpaca tradability gap becomes measurable. Measurable: (1) the next qualified-but-untradable name is added to the tally with its hypo P&L; (2) after several cases the running total shows whether the gap is a material or negligible drag on selection. If a future broker-block is logged only case-by-case in Missed Opportunities without advancing the running tally, the change didn't take.

**Updated process:** Entry flow checks `tradable` early and carries a `tradable=false` name forward without re-working it; morning eval keeps a running broker-block tally quantifying the Alpaca tradability-gap cost (data collection, no gate/threshold change). (No scanner parameters changed.)
