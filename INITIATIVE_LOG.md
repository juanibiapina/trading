...
### 2026-09-25 18:00 — Initiative 6 pre-entry liquidity audit; Initiative 3 full-window replay

**Evaluated:** The 15:00 hypothesis had **insufficient new pilot evidence**: the completed Sep 25 PM window did not admit SDEV (PM-only, thin, continuation gate failed); INLF, GLND, FTHM, and IFBD were AH-detected. The prior all-classification sensitivity still works on completed data, but the holdable-only +2.2% after assumed spread is conditioned on a retrospective label. Paper equity is $99,721.90 (-$278.10), flat. The prior Initiative 3 replay needed another complete AH/PM session before drawing a timing conclusion.

**Step taken:** **Pilot / Initiative 6:** re-ran `init6-pm-pilot.js` on completed Sep 25 SIP bars and instrumented a deterministic, log-only `log/init6-preentry-liquidity.csv` with the R+1/R+2 minimum trades and dollar volume known before each R+3 hypothetical entry, plus the later classification and modeled outcome. This tests whether the claimed edge survives causal liquidity selection, the highest unblocked expected $/time question. **Parallel Research / Initiative 3:** replayed eight Sep 24 AH candidates against complete Sep 25 premarket SIP bars; earlier entry matters for catching the overnight jump, but only if confirmation avoids fades. Verified `node --check scripts/init6-pm-pilot.js`, full script execution, and `git diff --check`. No orders, trading rules, or pulse timings changed.

**Result:** The holdable pilot remains **n=23, +97.5% modeled total / +4.2% gross per name, 18/23 positive**, or +2.2% after an assumed 2% spread; all classifications remain **n=32, +3.2% gross / +1.2% after assumed spread**. The pre-entry participation snapshot overlaps the hindsight classes: among 32 admitted cases, a diagnostic >=1,000-trade minimum in each confirmation bar retains all **3 uninvestable** and 18/23 holdable, with +3.19% gross on 22 retained versus +3.11% on 10 excluded. It does not identify a reliable investability gate and was not applied. The Initiative 3 second-bar test admitted **0/8**; the third-bar test admitted **3/8** (INLF, NCPL, FTHM), with only +1.3% mean entry-price edge versus the current grid. PM highs are optimistic ceilings, not executable exits. No proven higher-$-per-time strategy displaces the active pilot; its executable edge remains unproven.

**Hypothesis / next step:** A prospective cohort recorded before the PM entry window, with contemporaneous quotes and all gate decisions (including rejects), can expose universe and fillability bias that the post-session tracker cannot. Instrument that cohort in shadow mode next; do not promote the 1,000-trade diagnostic. Keep Initiative 3 confirmation research log-only and preserve existing scan timings pending Juan's veto. Reassess the core strategy at the October 1 net-positive deadline if paper equity stays negative.

**Needs from Juan:** Nothing new. Existing daily-email asks remain the four-scan retirement veto and review of the Initiative 3 exit proposal; Initiative 2 broker access is deferred at Juan's request.
...
**Step taken:** Re-ran `node scripts/init6-pm-pilot.js`; replayed seven Sep 23 AH candidates with `node scripts/ah-5m-confirmation-replay.js` against Sep 24 premarket bars; generated `reports/2026-09-23/index.html`; compiled the report builder and checked its local chart links; committed and pushed as `fc058f5`. Pages run **36003309053** succeeded, and the report, report index, and all three chart assets returned HTTP 200. No live orders, trading rules, or scan/evaluation times changed.

**Result:** Initiative 6 remains **n=23, SUM +97.5%, mean +4.2%/name, median +10.0%, positive 18/23, fade-tail 4/23 (17%)**; estimated +2.2%/name after ~2% spread remains hypothetical versus the live baseline of 0%. Sep 24's GCTK and WETO PM-open rows were AH-detected; SPHL was uninvestable, so no new pilot case qualified. Paper equity is **$99,721.90 (-$278.10)** and the account is flat. Initiative 3's partial replay admitted **2/7** on the 2-bar test (GCTK, WETO) and **3/7** on the 3-bar test (adds NCPL). Their average entry-price advantage over the current grid was **+13.2%** and **+1.9%**, respectively; NCPL's third-bar entry was -20.8% to PM open and only +4.0% to the observed PM high. The replay used bars available at 09:02 ET, before the 09:30 ET premarket cap, so this evidence is provisional and does not support a gate or cadence change. The new Sep 23 Pages report contains three charts and is live.

**Hypothesis / next step:** At the 18:00 CEST checkpoint, rerun the Sep 23 replay after the full premarket window and determine whether the 2-bar entry advantage repeats without the GCTK outlier or admits additional fades. Keep Initiative 6 log-only and rerun on the next verified holdable PM-only gapper. Continue publishing each chart-bearing daily report and check its public assets after deployment.

**Needs from Juan:** No new asks. Existing email-routed asks remain: veto the proposed retirement of the 22:05/22:10/22:20/22:25 CET observation scans; provide IBKR/Webull access for the broker fill test; review the proposed Initiative 3 +10% premarket exit.
...
**Step taken:** **Pilot / Initiative 6 plus parallel Research / Initiative 3 and build / Initiative 5.** Re-ran `node scripts/init6-pm-pilot.js`; replayed WHLR, IPDN, SQFT, BFRG, DCOY, and HAO from the Sep 22 AH session through `node scripts/ah-5m-confirmation-replay.js`; read the live scheduler and the Sep 22 scheduler-session traces; checked current Alpaca paper equity; and generated `reports/2026-09-22/index.html`. Both replay scripts executed successfully, `python3 -m py_compile scripts/generate-html-report.py` passed, and a local HTML check confirmed both TOPS/WHLR chart links resolve. No orders, live rules, or existing trading-pulse times changed.

**Result:** Init 6 remains **n=23, SUM +97.5%, mean +4.2%/name, median +10.0%, positive 18/23, fade-tail 4/23 (17%)**; full-window PM-last mean is **-7.3%**, or estimated **+2.2%/name net of 2% spread**. Init 3's 2-bar and 3-bar tests each admitted only SQFT (fade) and DCOY (winner), **2/6**; both rejected WHLR, IPDN, BFRG, and HAO. Entry-price edge versus the existing grid averaged **-4.6% (2-bar)** and **-10.8% (3-bar)**, so this session does not support denser timing or a hard-gate change. The Sep 22 log records **2/7 core scheduled AH scans**. Session traces show the 22:25 CET job made nine assistant tool calls before `Codex error: The usage limit has been reached`; 22:30, 22:45, 23:00, 23:30, 00:00, and 00:30 then failed immediately with the same error, although all jobs exist in `scheduler.json`. The proposed resource fix is a Juan-vetoed trim of the four added scans (22:05, 22:10, 22:20, 22:25), preserving 22:00/22:15/22:30/22:45 and every late entry scan; no schedule was changed. The most recent Alpaca paper equity is **$99,721.90, down $278.10 from $100,000**; TOPS realized **-$44.80 (-50%)**. Initiative 5's Sep 22 report was generated with two charts and validated locally.

**Hypothesis / next step:** Keep the Init 6 pilot log-only and evaluate the next verified PM-only holdable; no fresh Sep 23 case changed the estimate. For Initiative 3, route the proposed four-pulse trim to Juan's daily email for veto; if he approves, verify the 22:30-00:30 scans execute under the reduced opening cadence, and keep collecting full AH replay sessions. The latest replay does not show an early-entry advantage. At the 18:00 CET checkpoint, seed TOPS's Sep 23 post-exit premarket bars into Initiative 3 only after the 09:30 ET window is complete. For Initiative 5, the Sep 22 Pages report and both chart URLs returned HTTP 200 after push (workflow run 35866528073 succeeded); use the report surface on the next chart-bearing cycle and collect review feedback. The account remains net-negative with the Oct 1 checkpoint approaching, so preserve late scan coverage over extra observation rounds.

**Needs from Juan:** one new veto decision, routed to the daily email: retire the four 22:05/22:10/22:20/22:25 CET observation pulses and retain the 15-minute grid plus all late scans. This is not applied until Juan's veto window. Standing asks remain an IBKR paper account plus consolidated extended-hours US-equities data/API access (or Webull credentials) for Initiative 2, and review of the Init 3 plain +10% premarket sell-limit proposal.
...
**Result:** Initiative 6 remains **n=23, SUM +97.5%, mean +4.2%/name, median +10.0%, positive 18/23, fade-tail 4/23 (17%)**; the PM-last comparison is **-7.3% mean**, for an estimated **+2.2%/name net of 2% spread**. Initiative 5 now has an automated Pages delivery workflow; after enabling the repository's Pages workflow source, the deployment completed successfully and the public report URLs returned HTTP 200. No new evidence justifies changing Initiative 3's cadence or confirmation gate.
...
**Step taken:** **Pilot / Initiative 6 plus parallel Instrument / Initiative 3.** (1) Re-ran `node scripts/init6-pm-pilot.js` over the 48-candidate tracker. (2) Added four new log-only `post-market-scan.md` scheduler pulses at 22:05, 22:10, 22:20, and 22:25 CET (16:05, 16:10, 16:20, and 16:25 ET), completing five-minute coverage from 16:00 through 16:30 ET. These are before the 23:00 CET learning-phase entry window, so they do not change entry behavior or submit orders. (3) Re-ran `ah-5m-confirmation.js` on nine 2026-09-16 candidates to establish the next comparison seed.

**Result:** Init 6 remains unchanged: **n=21, SUM +77.5%, mean +3.7%/name, median +10.0%, positive 16/21, fade-tail 4/21 (19%)**, with 27 candidates gate-skipped and no new PM-only row. The fresh nine-name confirmation check returned **2 YES / 7 NO**: DAIC confirmed at 17:30 ET with a 724.8x local-volume ratio, and YFOR confirmed at 17:15 ET with 4.2x; the other seven failed the confirmation. The new cadence costs **+4 observation rounds/day**, taking the schedule from 9 to 13 scans, without changing any existing scan time. The DTSS/YFOR Init 3 exit seed is data-blocked until the SIP premarket window closes and the conflicting YFOR peak is resolved.

**Hypothesis / next step:** The five-minute opening grid will reduce ignition-to-observation lag and capture the first two or three rising bars without adding false entry behavior; measure that on the next complete AH session, including per-candidate ignition time, first available observation, CONFIRM-3 result, and later outcome. Re-run the Init 6 pilot when a fresh `footprint=none` holdable gapper appears. After complete 2026-09-17 SIP data is available, append DTSS/YFOR only if their post-exit peaks and fallback closes reconcile; do not let the position-evaluation discrepancy contaminate the sim.

**Needs from Juan:** no new ask. Standing asks remain: provide an IBKR paper account plus consolidated extended-hours US-equities data/API access (or Webull EU credentials) for Init 2's fill test, and review the proposed Init 3 resting +10% premarket sell-limit exit. The four new opening pulses are log-only and were added without changing an existing trading scan.


### 2026-09-16 — Initiative 6 pilot: fresh holdable PM-only gapper MEDS (+68.5%) gate-skipped as a VWAP-decline false-negative; entered set holds n=21, +3.7%/name (net ~+1.7%), tail 4/21 (19%). Init 3 UNBLOCKED — VEEA/WAFU/YFOR seeded (n=41), plain +10% limit still best. Init 2 blocked on Juan

**Evaluated:** The 2026-09-15 step **worked.** Its hypothesis for today was explicit: keep seeding holdable PM-only gappers and SIP-resolving `unknown` footprints (Init 6), and *resume Init 3 seeding once VEEA (or any position) exits.* Both came true today — one fresh holdable PM-only gapper (MEDS) logged for Init 6, and VEEA exited at the 09-16 04:32 ET pulse (alongside WAFU/YFOR) unblocking Init 3. The 09-15 catastrophe-stop rejection stands (no reason to revisit; today's Init 3 OCO-floor variants remain within noise, consistent). Init 2 remains blocked on Juan (needs an account).

**Step taken:** **Pilot / Initiative 6 (active slot) + parallel low-risk Initiative 3.** (1) One fresh holdable footprint=none PM-only gapper logged 09-16 (MEDS +68.5%, DataMEDS AI 1.5M-float squeeze, genuine multi-hour liquid exit window). Re-ran the full `init6-pm-pilot.js` (48 candidates, `node --check` clean, deterministic 21-row ledger). (2) For Init 3, pulled the three exit fills from `broker.js orders all --json` (VEEA $5.22, WAFU $1.71, YFOR $1.67, all filled 08:32:11Z = 04:32 ET), measured each post-exit premarket SIP peak (capped 09:30 ET), appended all three to `peak-seeking-exit-sim.js` (n=38 -> n=41) and `premarket-exit-gap.csv`, re-ran. All log-only, no orders. Init 2 needs a Juan account before the AKAN/SHPH/GIPR fill test can run.

**Result:** **Init 6 pilot:** **MEDS was SKIPPED by the continuation gate** — its 04:10 ET ignition bar (12,689 trades) wicked to $2.90/$3.24, but the R+2 bar (04:20) per-bar VWAP $2.83 fell below R+1's $2.94×0.98 ($2.88), tripping the **VWAP-non-declining** reject. This is a conservative **false-negative**: entered at the R+3 open (~$2.62), the resting +10% limit ($2.88) WOULD have filled (04:25 bar high $2.88; MEDS re-ramped to $4.21 by 06:10 ET). But the same VWAP-dip protection is what rejects AEHL-type wick-peak faders, so the skip is by design — MEDS joins the wick-rebuild false-negatives (WVVIP/RDIB) as the bounded cost of the conservative gate. Entered set unchanged at **n=21: SUM +77.5% / mean +3.7% / median +10.0% / positive 16/21**, net of ~2% spread ~+1.7%/name; fade-tail 4/21 (BIVI, MIMI-2, BNC, AEHL) = 19%, unchanged. **Init 3:** all three new seeds are dead/fading post-exit books with lim10 unfilled — VEEA (a Grade-A trail-stop win exit) peak $5.33 08:45 ET (+2.1%), PM-last $5.15 (-1.3%); WAFU faded, PM peak only $1.70 (below the $1.71 exit), PM-last $1.54 (-9.9%); YFOR peak $1.80 09:35 ET (+7.8%), PM-last $1.7008 (+1.8%). Re-ran the sim: plain **+10% sell-limit stays the best rule at n=41: SUM +91.4% / mean +2.2%, positive 25/41** (down from n=38's +100.7/+2.6; WAFU -9.9% the main drag, VEEA a small drag, YFOR a small positive). Init 2 blocked on Juan. **Deadline context:** account equity $99,822.72 (-$177 net); today's live cycle earned +$39.36 net via the AH->PM core (VEEA's first live Grade-A trail-stop win, +$51.52), but the account is still net-negative with the ~10-01 target ~2 weeks out.

**Hypothesis / next step:** Init 6 — the pilot edge is stable (n=21, +3.7%/name, 16/21 positive, tail 19%, two converging studies); the conservative gate keeps correctly rejecting weak-hold shapes (MEDS VWAP-dip, LBGJ/SXTC/TNON/YMAT wick-peaks) at the bounded cost of a few false-negatives. Keep seeding holdable PM-only gappers and SIP-resolving `unknown` footprints; the live blocker stays the broker (Init 2 account) plus Juan's veto on the entry pulse. Init 3 — keep seeding held-name exits; the +10%-limit edge is robust at n=41 (positive 25/41), dead/fading books contribute ~0 to a drag. Init 2 — run the AKAN/SHPH/GIPR fill test once Juan opens an account. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** two open asks unchanged (no new one), flagged as the deadline bottleneck (~10-01 net-positive target, ~2 weeks out). (1) Init 2 — open an **IBKR paper account** + a **US-equities market-data subscription with extended-hours consolidated quotes** (not IEX-only) + API access; OR register at **developer.webull.eu** and share the **App Key/Secret** — needed to run the AKAN/SHPH/GIPR fill test and to eventually fill the PM-only gappers the pilot proves out. (2) Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback; **no stop** — the 09-15 intrabar test rejected the catastrophe-stop at every width). Evidence: +91.4% total / +2.2%/name over 41 seeds, positive 25/41. Veto window, not applied.

---

### 2026-09-15 — Initiative 6 pilot: fresh admit MTEN filled the +10% limit (n=21, +3.7%/name, net ~+1.7%, tail 4/21=19%); the 09-14 catastrophe-stop hypothesis is REJECTED by an intrabar test (stop hurts at every width) — exit proposal stays a plain +10% limit, no stop. Init 3 data-blocked (VEEA held), Init 2 blocked on Juan

**Evaluated:** The 2026-09-14 step **worked** (AEHL correctly SIP-resolved unknown->none, admitted, faded — a documented gate false-positive; entered n=20). Its hypothesis for today was explicit: *if the fade-tail holds near/above ~20-25%, promote the ~-15% catastrophe-stop from optional into the exit proposal.* Tested that hypothesis directly this run (below) — and **rejected it**. AEHL's 09-14 PM-last also recomputed -34.1% -> -32.6% on fuller 1-min bars (no change to the conclusion). Init 3 is genuinely data-blocked: VEEA is held day 1 (entered 09-14 $3.38, +19.8% at the 09-15 14:30 eval, HOLD), so no held-name exit seeded at the 09-15 pulse. Init 2 remains blocked on Juan (needs an account).

**Step taken:** **Pilot / Initiative 6 (active slot) + a low-risk exit-rule refinement.** (1) One fresh holdable footprint=none PM-only gapper logged 09-15 (MTEN +15.9%, Mingteng Intl 6.1M-float micro-cap). Re-ran the full `init6-pm-pilot.js` (47 candidates, `node --check` clean; re-ran the full pilot after an ad-hoc single-name run to restore the deterministic 21-row ledger). (2) To settle the 09-14 catastrophe-stop hypothesis with evidence rather than the naive PM-last-capping mental model, built `scripts/init6-catastrophe-stop.js` (log-only, no orders/ledger/pulse change): it reuses the exact pilot gate + universe, then walks 1-min SIP bars in time order applying BOTH the resting +10% sell-limit AND a -N% catastrophe-stop, whichever triggers first. Swept N = 15/20/25/30% under both a conservative (stop wins same-bar ties) and an optimistic (limit wins) tie-break. All log-only, no orders.

**Result:** **Init 6 pilot:** **MTEN was admitted** (entered $1.14 at 04:40 ET) and its early ramp **filled the resting +10% limit ($1.25)** intrabar = a win, even though it drifted to PM-last -9.6% (the peak-seeking case the resting limit is built for). Entered set moves n=20 -> **n=21: SUM +77.5% / mean +3.7% / median +10.0% / positive 16/21**, vs a PM-last floor of -7.3% and a do-nothing baseline of 0%; net of ~2% spread **~+1.7%/name (up from +1.3%)**. The **fade-tail eases to 4/21 (BIVI -27.1, MIMI-2 -6.9, BNC -14.7, AEHL -32.6) = 19%** — MTEN added a limit-win not a fade, so the ratio slips back from 20%. **Catastrophe-stop test — REJECTED:** at every width the stop is net-negative vs the plain +10% limit (+1.7%/name): **-15% -0.4, -20% +0.9, -25% +0.5, -30% -0.1** (tie-break-independent — the conservative and optimistic runs gave identical numbers). Cause: these micro-float PM gappers whipsaw deep intrabar then re-ramp, and the resting +10% limit's whole edge is *staying in all premarket to catch the re-ramp*. The decisive case is **WBUY (07-23)**: entered $1.28, dumped to ~$0.84 (-34%), then re-ramped to $1.68 filling the +10% limit at 08:32 ET — any stop bails at the whipsaw low and misses the re-ramp (verified on real SIP bars; WBUY and WHLR are genuine time-ordered pre-empts, not same-bar artifacts). **Decision: DROP the optional catastrophe-stop; the exit rule stays a plain resting +10% sell-limit + PM-last fallback.** This simplifies the standing exit proposal to Juan (was "+10% limit, optional wide ~-15% catastrophe-stop"). Init 3 unchanged at n=38 (data-blocked); Init 2 blocked on Juan.

**Hypothesis / next step:** Init 6 — the pilot edge is stable (n=21, +3.7%/name, 16/21 positive, tail 19%, two converging studies) and the exit rule is now simplified (no stop). Keep seeding holdable PM-only gappers and SIP-resolving `unknown` footprints; the live blocker stays the broker (Init 2 account) plus Juan's veto on the entry pulse. Init 3 — resume seeding held-name exits once VEEA (or any position) exits; +10%-limit edge robust at n=38 (positive 24/38). Init 2 — run the AKAN/SHPH/GIPR fill test once Juan opens an account. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** two open asks unchanged (no new one), flagged as the deadline bottleneck (~10-01 net-positive target, ~2 weeks out). (1) Init 2 — open an **IBKR paper account** + a **US-equities market-data subscription with extended-hours consolidated quotes** (not IEX-only) + API access; OR register at **developer.webull.eu** and share the **App Key/Secret** — needed to run the AKAN/SHPH/GIPR fill test and to eventually fill the PM-only gappers the pilot proves out. (2) Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback). Evidence: +100.7% total / +2.6%/name over 38 seeds, positive 24/38. **Simplification from today's test:** the previously-optional ~-15% catastrophe-stop is now **dropped** — an intrabar sim shows it is net-negative at every width because it pre-empts the deep-dip-then-re-ramp winners the resting limit is built to catch. The clean ask is the plain +10% limit, no stop. Veto window, not applied.

---

### 2026-09-14 — Initiative 6 pilot: fresh out-of-sample admit AEHL (footprint SIP-resolved unknown->none) is a HARD FADE; the gate false-positive drops the entered edge to n=20 / +3.3%/name (net ~+1.3%), fade-tail 4/20 (20%). Init 3 data-blocked (flat book), Init 2 blocked on Juan

**Evaluated:** The 2026-09-11 step **worked** (LBGJ + SXTC correctly gate-skipped as wick-peaks; entered n=19; Init 3 seeded UFG/CULP to n=38). No caveat to reconcile today. This is the first run since Fri 09-11 (Sat/Sun markets closed), so the pilots looked at first glance data-blocked — but Mon 09-14's PM-open scan logged three `footprint=unknown` gappers (Friday evening's AH scan was never captured, so the pulse could not classify them), which are resolvable against Friday SIP AH bars rather than simply excluded. Init 3 is genuinely data-blocked: flat book since the 09-11 UFG/CULP exits, no held-name exit at the 09-14 pulse. Init 2 remains blocked on Juan (needs an account).

**Step taken:** **Pilot / Initiative 6 (active slot) + footprint resolution.** Pulled Friday 09-11 AH (16:00-20:00 ET) SIP bars for all three 09-14 `unknown` gappers via `broker.js bars`. **VSME** (274K sh/1489 tr at ~$1.3) and **CRBP** (thousands of trades at ~$10.3) both had real Friday AH sessions -> reclassified `ah-detected` in `pm-open-scan.csv` (AH-footprint names we merely failed to scan, correctly excluded from the PM-only pilot); **AEHL** returned ZERO Friday AH bars -> reclassified `none`, a genuine PM-only gapper (same class as its 08-31 weekend-gap appearance) and eligible for the pilot. `node --check` clean; re-ran the full `init6-pm-pilot.js` (46 candidates). All log-only, no orders. Init 3 had no fresh held-name exit to seed (flat book); Init 2 needs a Juan account before the AKAN/SHPH/GIPR fill test can run.

**Result:** **AEHL was ADMITTED** by the continuation gate (entry $9.12 at 04:15 ET) and **faded to PM-last -34.1%** — never reached the resting +10% limit; verified the bleed to ~$6 on real 1-min volume through premarket (not a stray print). This is a **gate false-positive**: AEHL's 04:00 bar wicked to $10.61 then closed $8.69, and its R+1/R+2 closes ($9.20/$9.12) held just above 80% of the wick high ($8.49), so the wick-high gate admitted it — then it bled -34% all premarket. Entered set moves n=19 -> **n=20: SUM +65.9% / mean +3.3% / median +10.0% / positive 15/20**, vs a PM-last floor of -7.3% and a do-nothing baseline of 0%; net of ~2% spread **~+1.3%/name (down from +3.3%)**. The **fade-tail grows to 4/20 (BIVI -27.1, MIMI-2 -6.9, BNC -14.7, AEHL -34.1) = 20%**, nearing the ~25% that erodes the edge below the spread. **AEHL is the strongest evidence yet FOR the (currently optional) ~-15% catastrophe-stop on the exit rule**: a -15% stop caps AEHL at -15% instead of -34%, lifting the entered mean back to ~+4.3%/name. So the wick-high gate cuts both ways: it false-negatives rebuilders (WVVIP/RDIB) and now false-positives a wick-peak fader (AEHL). Init 3 unchanged at n=38 (data-blocked); Init 2 blocked on Juan.

**Hypothesis / next step:** Init 6 — the pilot edge thinned this run to ~+1.3%/name net and the fade-tail hit 20%; the money-fast lever is now capping the fade-tail. If it holds near/above ~20-25%, promote the ~-15% catastrophe-stop from *optional* into the exit proposal (it converts AEHL-type -34% bleeds to -15% and restores the mean). Keep seeding holdable PM-only gappers and SIP-resolving `unknown` footprints. Init 3 — resume seeding held-name exits once a position exits; +10%-limit edge robust at n=38 (positive 24/38). Init 2 — run the AKAN/SHPH/GIPR fill test once Juan opens an account. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** two open asks unchanged (no new one), flagged as the deadline bottleneck (~10-01 net-positive target, ~2.5 weeks out). (1) Init 2 — open an **IBKR paper account** + a **US-equities market-data subscription with extended-hours consolidated quotes** (not IEX-only) + API access; OR register at **developer.webull.eu** and share the **App Key/Secret** — needed to run the AKAN/SHPH/GIPR fill test and to eventually fill the PM-only gappers the pilot proves out. (2) Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback). Evidence: +100.7% total / +2.6%/name over 38 seeds, positive 24/38. **New nuance from today's AEHL fade:** the wide ~-15% catastrophe-stop (previously optional) now looks worth including — it caps the fade-tail the +10% limit cannot dodge. Veto window, not applied.

---

### 2026-09-11 — Initiative 6 pilot: no fresh admit (LBGJ + SXTC both gate-skipped as wick-peaks); entered n=19 holds +5.3%/name, tail 3/19 (16%). Init 3 unblocked (UFG-2, CULP seeded, n=38); Init 2 blocked on Juan

**Evaluated:** The 2026-09-10 step **worked** (EHGO admitted + filled the +10% limit, TNON gate-skipped as a wick-peak, entered n=19). No caveat to reconcile — its only wrinkle today is EHGO 09-10's PM-last row recomputing -2.9% -> +3.9% on fuller 1-min bars, but EHGO's +10% limit-win was already locked so the pilot SUM is unchanged. Init 3 had been data-blocked three runs (CULP/UFG held); both exited today at the 09-11 04:30 ET pulse, so Init 3 is now seedable. Init 2 remains blocked on Juan (needs an account for the fill test).

**Step taken:** **Pilot / Initiative 6 (active slot) + parallel low-risk Initiative 3.** Fri 09-11 logged three PM-only gappers: AENT +97.8% (uninvestable, straight-down spike-fade after the 04:10 $28.99 peak), LBGJ +29.6% and SXTC +21.9% (both footprint=none, in the pilot universe). Re-ran `init6-pm-pilot.js` (45 candidates, `node --check` clean). For Init 3, pulled the two exit fills from `broker.js orders all --json` (UFG $0.6055, CULP $3.69, both filled 08:30:40Z = 04:30 ET), measured each post-exit premarket SIP peak (capped 09:30 ET), appended both to `peak-seeking-exit-sim.js` (n=36 -> n=38) and `premarket-exit-gap.csv`, re-ran. All log-only, no orders. Init 2 needs a Juan account before the AKAN/SHPH/GIPR fill test can run.

**Result:** **Init 6:** **both LBGJ and SXTC were skipped by the continuation gate** — each is a first-bar wick then a flat plateau at gap-mid (LBGJ $3.25 wick -> $2.49-2.58 closes ~gap-mid $2.595; SXTC $2.74 wick -> $2.16-2.38 closes ~gap-mid $2.335), the weak-hold wick-peak shape the R+1/R+2 80%-of-high hold rejects by design (same as TNON/YMAT/GMEX). Entered set stays **n=19: SUM +100.0% / mean +5.3% / median +10.0% / positive 15/19**, vs a PM-last floor of -5.8% and a do-nothing baseline of 0%; net of ~2% spread ~+3.3%/name. Fade-tail 3/19 (BIVI -27.1%, MIMI-2 -6.9%, BNC -14.7%) = 16%, unchanged. **Init 3:** both new seeds are thin dead-book stallers with lim10 unfilled — UFG exit $0.6055 -> peak $0.616 (+1.7%, 2-25 tr/bar), lim10 ($0.666) unfilled, PM-last $0.57 (-5.7%); CULP exit $3.69 -> peak $3.785 (+2.6%, 1-5 tr/bar), lim10 ($4.059) unfilled, PM-last +2.6%. Re-ran the sim: plain **+10% sell-limit stays the best rule at n=38: SUM +100.7% / mean +2.6%, positive 24/38** (down from n=36's +104.0/+2.9; UFG a small drag, CULP a small positive, conclusion unchanged). Init 2 blocked on Juan.

**Hypothesis / next step:** Init 6 — the wick-high gate keeps correctly rejecting wick-peak weak-holds (LBGJ, SXTC, TNON, YMAT); edge stable at n=19, +5.3%/name, 15/19 positive, tail 16%, two converging studies. Keep seeding holdable PM-only gappers; the pilot is near proposable but the live blocker is the broker (Init 2 account). Init 3 — keep seeding held-name exits; +10%-limit edge robust at n=38 (positive 24/38), dead books contribute ~0. Init 2 — run the AKAN/SHPH/GIPR fill test once Juan opens an account. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** two open asks unchanged (no new one), flagged as the deadline bottleneck (~10-01 net-positive target). (1) Init 2 — open an **IBKR paper account** + a **US-equities market-data subscription with extended-hours consolidated quotes** (not IEX-only) + API access; OR register at **developer.webull.eu** and share the **App Key/Secret** — needed to run the AKAN/SHPH/GIPR fill test and to eventually fill the PM-only gappers the Init 6 pilot proves out. (2) Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback; optional wide ~-15% catastrophe-stop). Evidence: +100.7% total / +2.6%/name over 38 seeds, positive 24/38. Veto window, not applied.

---

### 2026-09-10 — Initiative 6 pilot: fresh out-of-sample admit (EHGO filled the +10% limit) + TNON gate-skipped as a wick-peak; entered n=19 firms to +5.3%/name, fade-tail 3/19 (16%). Init 3 data-blocked (CULP/UFG held), Init 2 blocked on Juan

**Evaluated:** The 2026-09-09 step **worked** (two wick-peak names YMAT/DPU correctly gate-skipped; wick-denominator re-test confirmed keeping the conservative wick-high gate; entered n=18 stable). No caveat to reconcile today — the 09-09 rows were already on full 1-min data. Init 3 was data-blocked on 09-09 (flat book); today CULP + UFG are still held at the 09-10 14:30 pulse with no exit, so Init 3 stays data-blocked. Init 2 remains blocked on Juan (needs an account for the fill test).

**Step taken:** **Pilot / Initiative 6 (active slot).** Thu 09-10 logged two holdable footprint=none PM-only gappers: EHGO +15.2% (recurring registered-direct-offering dilution name, prior admits 07-13/07-16/07-23/07-28/09-01) and TNON +47.1% (Tenon Medical, 551K-float squeeze). Re-ran `init6-pm-pilot.js` (43 candidates, `node --check` clean). All log-only, no orders. Init 3 had no fresh held-name exit to seed (CULP/UFG both HOLD); Init 2 needs a Juan account before the AKAN/SHPH/GIPR fill test can run.

**Result:** **EHGO was admitted** by the continuation gate (entered $1.03 at 04:30 ET after a flat 04:00-04:10 open, ignition 04:15) and its early ramp **filled the resting +10% limit ($1.13)** intrabar = a win at the exit rule, even though it later drifted to PM-last -2.9% (the exact peak-seeking case the resting limit is built for). **TNON was skipped** — its $4.22 first-bar wick then plateau ~$3.6 (>15% below the wick) is the GMEX/YMAT wick-peak shape the R+1/R+2 80%-of-high hold rejects by design. Entered set moves n=18 -> **n=19: SUM +100.0% / mean +5.3% / median +10.0% / positive 15/19**, vs a PM-last floor of -6.2% and a do-nothing baseline of 0%; net of ~2% spread ~+3.3%/name. **Fade-tail eases to 3/19 (BIVI -27.1%, MIMI-2 -6.9%, BNC -14.7%) = 16%** — EHGO added a limit-win not a fade and n grew, so the ratio slips from 17% and stays under the ~25% that would erode the edge below the spread. This is the second EHGO admit (07-23 held +59% PM-last); the recurring dilution names keep re-igniting and the gate keeps catching the holdable ones. Init 3 unchanged at n=36 (data-blocked); Init 2 blocked on Juan.

**Hypothesis / next step:** Init 6 — the pilot edge is stable and firming (n=19, +5.3%/name, 15/19 positive, tail 16%, two converging studies); keep seeding holdable PM-only gappers and hold the conservative wick-high gate (it correctly skipped TNON). With the deadline ~3 weeks out, the pilot is near proposable but the live blocker is the broker (Init 2 account). Init 3 — resume seeding held-name exits once CULP/UFG (or any position) exits; +10%-limit edge robust at n=36. Init 2 — run the AKAN/SHPH/GIPR fill test once Juan opens an account. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** two open asks unchanged (no new one), flagged as the deadline bottleneck. (1) Init 2 — open an **IBKR paper account** + a **US-equities market-data subscription with extended-hours consolidated quotes** (not IEX-only) + API access; OR register at **developer.webull.eu** and share the **App Key/Secret** — needed to run the AKAN/SHPH/GIPR fill test and to eventually fill the PM-only gappers the Init 6 pilot proves out. (2) Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback; optional wide ~-15% catastrophe-stop). Evidence: +104.0% total / +2.9%/name over 36 seeds, positive 23/36. Veto window, not applied.

---

### 2026-09-09 — Initiative 6 pilot: two fresh names (YMAT +72.8%, DPU +45.3%) both gate-skipped as wick-peaks; entered n=18 holds +5.0%/name, tail 3/18 (17%); wick-denominator re-test (08-26 trigger met) confirms keeping the conservative wick-high gate

**Evaluated:** The 2026-09-08 step **worked** (universe widened to holiday PM-only gappers, BNC/ISPC admitted, entered n=18). Its caveat was that 09-08 rows were computed while premarket was still live, so BNC's PM-last could shift by the 09:30 cap. Confirmed today on fuller data: BNC refined -15.3% -> **-14.7%** (still a fade), ISPC PM-last -2.6% (its +10% limit-win was already locked). No change to the conclusion. Init 3 was data-blocked (no held-name exit since 09-04); today `OPEN_POSITIONS.md` is still flat, so Init 3 stays blocked. Init 2 remains blocked on Juan (needs an account for the fill test).

**Step taken:** **Pilot / Initiative 6 (active slot) + gate sensitivity re-test.** Wed 09-09 logged three holdable PM-only gappers: YMAT +72.8% and DPU +45.3% (both footprint=none, in the pilot universe) and FGL +43.2% (ah-detected, outside it). Re-ran `init6-pm-pilot.js` (41 candidates). Separately, YMAT/DPU are the **3rd and 4th** wick-then-rebuild holdables the wick-high gate has now rejected (after WVVIP 08-25, RDIB 08-26), meeting the 08-26 trigger to re-test a holdable-gated close-denominator, so re-ran `init6-gate-denom-test.js` (footprint=none, n=61). All log-only, no orders. (Note: ad-hoc single-name runs of the pilot overwrite the ledger, so re-ran the full pilot to restore the 18-row ledger — verified.)

**Result:** **Init 6 pilot:** **both YMAT and DPU were skipped by the continuation gate** — YMAT wicked to $3.28 then plateaued ~25% below it (GMEX-style wick-peak); DPU had only 3 real bars (late 04:35 ramp + SIP blocking the recent ~15 min), so neither holds R+1/R+2 against the wick high. Entered set stays **n=18: SUM +90.0% / mean +5.0% / median +10.0% / positive 14/18**, vs a PM-last floor of -6.4% and a do-nothing baseline of 0%; net of ~2% spread ~+3.0%/name. Fade-tail 3/18 (BIVI -27.1%, MIMI-2 -6.9%, BNC -14.7%) = 17%. **Denominator re-test:** close-denom recovers 4 clean holdable +10% winners (WXM, RDIB, YDDL, DPU) but **also admits YMAT as a -23% fade** into the holdable set (its early plateau collapsed after 04:45 to PM-last -23%) AND balloons the uninvestable false-positive bucket from -16.3%/n7 to -61.7%/n13 (LICN -50%, DXST -15%, etc.). So within holdable it lifts SUM (+94.8 -> +111.7) but dilutes the mean (+5.9 -> +5.3) and adds a fade to the tail — not a free win, same conclusion as 08-26. For the 09-09 pair specifically, keeping the wick-high gate was net **+13%** (correctly avoided YMAT -23%, missed DPU +10%). **Decision: keep the conservative wick-high gate.** Init 3 unchanged at n=36 (data-blocked); Init 2 blocked on Juan.

**Hypothesis / next step:** Init 6 — the wick-high gate's rejection of wick-peak fades (YMAT) is the protection the pilot wants before going live; keep it. Revisit the close-denominator only if clean wick-rebuild winners (WXM/RDIB/YDDL/DPU-type) start clearly outnumbering YMAT-type wick-fades. Keep seeding holdable PM-only gappers; edge is stable (n=18, +5.0%/name, tail 17%, two converging studies). Init 3 — resume seeding held-name exits once a position exits; +10%-limit edge robust at n=36. Init 2 — run the AKAN/SHPH/GIPR fill test once Juan opens an account. **Deadline pressure:** with the ~10-01 net-positive target ~3 weeks out and the live cycle entering none of these PM-only gappers, the two standing asks are the money-fast bottleneck.

**Needs from Juan:** two open asks unchanged (no new one), now flagged as the deadline bottleneck. (1) Init 2 — open an **IBKR paper account** + a **US-equities market-data subscription with extended-hours consolidated quotes** (not IEX-only) + API access; OR register at **developer.webull.eu** and share the **App Key/Secret** — needed to run the AKAN/SHPH/GIPR fill test and to eventually fill the PM-only gappers the Init 6 pilot proves out. (2) Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback; optional wide ~-15% catastrophe-stop). Evidence: +104.0% total / +2.9%/name over 36 seeds, positive 23/36. Veto window, not applied.

---

### 2026-09-08 — Initiative 6 pilot: universe widened to holiday PM-only gappers; two fresh admits (ISPC +10% limit-win, BNC fade), WETO-2 self-corrects to a win; entered n=18, edge +5.0%/name, fade-tail 3/18 (17%)

**Evaluated:** The 2026-09-07 step **worked** (broker-alternatives shortlist written, IBKR primary / Webull EU backup, blocked on Juan for an account). It had no fresh market-data evaluation to make today. This is the first run since Fri 09-04 (Mon 09-07 was Labor Day, market closed). Fresh 09-08 data landed for the Init 6 pilot; Init 3 is still data-blocked (no held-name exit since 09-04, `OPEN_POSITIONS.md` flat, no sells since the 09-04 04:31 ET pulse).

**Step taken:** **Pilot / Initiative 6 (active slot) + log-only instrumentation.** Tue 09-08 PM had three holdable PM-only gappers with **no AH session at all** (Labor Day = the purest AH-scanner blind spot): BNC +68.2%, GMEX +35.1%, ISPC +26.0%. They were tagged footprint=`unknown` (no AH to check), which the pilot loader had always excluded. Widened `init6-pm-pilot.js` `loadHoldablePmOnly()` to also admit footprint=`unknown` rows explicitly flagged as a market **holiday** — verified only the 3 09-08 rows match (`grep` on the tracker), so the other 13 `unknown` data-gap rows stay excluded (clean, precise). `node --check` passes; re-ran the pilot. All log-only, no orders.

**Result:** **GMEX skipped** by the continuation gate (wick-peak plateau at gap-mid, correct rejection). **BNC admitted** (entered $6.32 at 04:15 ET) and **faded** — never reached the resting +10% limit ($6.95), drifted to PM-last -15.3%. **ISPC admitted** (entered $1.96 at 04:15 ET) and **filled the +10% limit** ($2.16) intrabar = a win. Separately, WETO-2 (09-04) reclassified from -1.8% to **+10.0%** on fuller 1-min bars now available (its +10% limit $4.35 filled intrabar). Entered set moves n=16 -> **n=18: SUM +89.4% / mean +5.0% / median +10.0% / positive 14/18**, vs a PM-last floor of -6.4% and a do-nothing baseline of 0%. Net of ~2% spread ~+3.0%/name. The **fade-tail is 3/18 (BIVI -27.1%, MIMI-2 -6.9%, BNC -15.3%) = 17%** — WETO-2 left the tail (corrected to a win), BNC replaced it, n grew, so the ratio eased from 19% and stays under the ~25% that erodes the edge below the spread. **Caveat:** 09-08 rows were computed while premarket was still live (~09:00 ET at run time), so BNC's PM-last may shift by the 09:30 cap; the ISPC/WETO-2 limit fills are locked. Init 3 unchanged at n=36 (data-blocked). Init 2 broker research blocked on Juan (needs an account for the fill test).

**Hypothesis / next step:** Init 6 — the holiday-widened universe adds genuine out-of-sample PM-only gappers (BNC/ISPC this run); keep seeding and track the fade-tail (17%, under ~25%). With n=18 / 14 positive / two converging studies the live PM-gapper pulse stays near proposable, hold until n grows and the tail stays contained. Init 3 — resume seeding held-name exits once a position exits; the +10%-limit edge is robust at n=36 (positive 23/36). Init 2 — run the AKAN/SHPH/GIPR fill test once Juan opens an IBKR paper (or Webull EU) account. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** two open asks unchanged (no new one). (1) Init 2 (to unblock the empirical fill test): open an **IBKR paper account** + a **US-equities market-data subscription that includes extended-hours consolidated quotes** (not IEX-only) + API access; OR register at **developer.webull.eu** and share the **App Key/Secret**. (2) The Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback; optional wide ~-15% catastrophe-stop). Evidence: +104.0% total / +2.9%/name over 36 seeds, positive 23/36. Veto window, not applied.

---

### 2026-09-07 — Initiative 2: broker-alternatives shortlist done (IBKR primary, Webull EU backup); pilots data-blocked over the weekend

**Evaluated:** The 2026-09-04 steps **worked** (Init 6 pilot admitted WETO-2, entered n=16; Init 3 seeded four exits, +10%-limit robust at n=36). Both had no fresh data to evaluate today: this is the first run since Fri 09-04 (Sat/Sun were the weekend, markets closed), so there is **no new market data** — no 09-05/06/07 log dir, `pm-open-scan.csv` still ends 09-04, and `OPEN_POSITIONS.md` is flat. So the active **Init 6 pilot** has no fresh holdable PM-only gapper to admit and **Init 3** has no held-name exit to seed. Both are data-blocked, not failed; re-running the sims would produce identical output. Advanced the highest-value unblocked non-pilot item instead.

**Step taken:** **Research / Initiative 2 (Juan's active 09-04 directive):** researched broker alternatives to Alpaca from primary sources (IBKR/Webull/Moomoo/Trading 212 docs + community reports) and wrote `INIT2_BROKER_ALTERNATIVES.md` — requirements, per-candidate evaluation against the decisive criterion (does the paper fill engine see a *consolidated* extended-hours quote for sub-$1 micro-floats), a recommendation, and an empirical fill-test protocol on the exact chronic-block names (AKAN/SHPH/GIPR). No account created (needs Juan), so this is the Research step that answers "which provider" as far as it can without an account.

**Result:** **Trading 212 REJECTED** — its API supports extended-hours *market* orders only, no limit orders (`invalid payload`); our micro-cap AH strategy is limit-only. **Moomoo/Futu** possible but EU-access doubtful (no EU retail entity; ETH paper needs a US-margin account + OpenD gateway). **Webull OpenAPI = backup** — EU developer portal confirmed (`developer.webull.eu`), official Python SDK, Jul-2026 API paper trading, native extended-hours; unconfirmed whether the EU entity's paper API exposes US micro-cap AH symbols, but lowest setup (pure REST). **IBKR paper = recommended primary** — EU-regulated + the eventual live path, and its paper fill engine simulates against the *real consolidated market data you subscribe to* (shareable with paper), so a non-IEX-only US-equities subscription lets AH micro-float limits fill exactly where Alpaca's free IEX feed cannot. A community report shows IBKR paper filling AH limit orders. This is the direct fix for the documented root cause (`INIT2_ALPACA_FILL_ROOTCAUSE.md`). The decisive fill test needs a live account/data, so it is now blocked on Juan.

**Hypothesis / next step:** IBKR paper (with a US extended-hours consolidated data subscription) will fill the AKAN/SHPH/GIPR AH limits Alpaca cannot; Webull EU is the fast fallback. Next run: resume the Init 6 pilot + Init 3 seeding when Mon 09-07 market data lands (a fresh holdable PM-only gapper / held-name exit); and, once Juan opens an account, wire a read-only `broker2.js` shim and run the fill-test protocol on the chronic-block names. Keep everything log-only, no live broker switch.

**Needs from Juan:** **one new ask + two standing.** NEW (Init 2, to unblock the empirical fill test): open an **IBKR paper account** and enable/share a **US-equities market-data subscription that includes extended-hours consolidated quotes** (not IEX-only) + API access (Client Portal Web API creds or a headless IB-Gateway); OR register at **developer.webull.eu** and share the **App Key/Secret**. This *replaces* the old 08-07 "pick Alpaca SIP / IBKR / modeled fills" ask — Juan already chose switch-broker on 09-04. STANDING (Init 3): the peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback; optional wide ~-15% catastrophe-stop). Evidence: +104.0% total / +2.9%/name over 36 seeds, positive 23/36. Veto window, not applied.

---

### 2026-09-04 — Initiative 6 pilot: fresh out-of-sample admit (WETO-2) FADED, entered n=16 eases to +5.2%/name, fade-tail 3/16 (19%). Initiative 3 seeded four exits (PLAG limit-win), n=36

**Evaluated:** The 2026-09-03 steps **worked.** Init 6's next step was to keep seeding holdable PM-only gappers and track the fade-tail ratio (2/15 = 13% after VIVK corrected out); Init 3's was to keep seeding held-name exits (+10%-limit edge robust at n=32). Both had fresh data today: one holdable footprint=none PM-only gapper logged 09-04 (WETO +23.9%), and four held names exited at the 09-04 04:31 ET pulse (PLAG $0.8441, GIPR $0.4811, TLYS $4.20, CHPT $9.32).

**Step taken:** **Pilot / Initiative 6 (active slot):** re-ran `init6-pm-pilot.js` (36 holdable candidates now). **WETO-2 was admitted** by the continuation gate (entered $3.95 at 04:20 ET) — a fresh out-of-sample entry on a recurring name. Ledger `log/init6-pm-pilot.csv` grows to 16 entered rows. **Parallel low-risk / Initiative 3:** pulled the four exit fills from `broker.js orders all --json` (all filled 08:30-08:31Z = 04:31 ET), measured each post-exit premarket SIP peak (capped 09:30 ET), appended all four to `peak-seeking-exit-sim.js` (n=32 -> n=36) and `premarket-exit-gap.csv`, re-ran. All log-only, no orders.

**Result:** **Init 6:** WETO-2 **faded** — never reached the resting +10% limit ($4.35), drifted to PM-last -1.8% (the same recurring name held +3.1% at 08-31, so this is a real out-of-sample fade). Entered set moves n=15 -> **n=16: SUM +83.0% / mean +5.2% / median +10.0% / positive 12/16**, vs a PM-last floor of -6.5% and a do-nothing baseline of 0%. Net of ~2% spread ~+3.2%/name. The **fade-tail grows to 3/16 (BIVI -27.1%, MIMI-2 -6.9%, WETO-2 -1.8%) = 19%** — still under the ~25% that would erode the edge below the spread, but worth watching. **Init 3:** PLAG is a live-book spike (exit $0.8441 -> PM peak $0.94 05:45 ET, **fills the resting +10% limit $0.9285**, PM-last +5.0%); TLYS/GIPR/CHPT are dead/stalled books (peaks +3.1% / +2.3% / below-exit, lim10 unfilled, small drags). Re-ran the sim: plain **+10% sell-limit stays the best rule at n=36: SUM +104.0% / mean +2.9%, positive 23/36** (up from n=32's +95.0/+3.0; PLAG adds a +10 win, three dead books a small drag).

**Hypothesis / next step:** Init 6 — the fade-tail is 3/16 = 19% after WETO-2 faded; keep seeding holdable PM-only gappers and track the ratio. With n=16 / 12 positive / two converging studies the live PM-gapper pulse stays near proposable, hold until n grows and the tail stays under ~25%. Init 3 — keep seeding held-name exits; the +10%-limit edge is robust at n=36 (positive 23/36) and live-book spikes (PLAG) keep filling the limit while dead books contribute ~0. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** two open asks unchanged (no new one). (1) The Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback; optional wide ~-15% catastrophe-stop). Evidence: +104.0% total / +2.9%/name over 36 seeds, positive 23/36. Veto window, not applied. (2) Still open from 08-07: the Initiative 2 broker/feed decision for the ~203% fill gap (Alpaca SIP ~$99/mo, IBKR paper, or modeled SIP fills).

---

### 2026-09-03 — Initiative 6 pilot: no fresh admit (GYGY gate-skipped), VIVK fade corrected to flat; entered n=15 firms to +5.7%/name, fade-tail back to 2/15 (13%). Initiative 3 seeded (UFG, GELS, n=32)

**Evaluated:** The 2026-09-02 steps **worked.** Init 6's next step was to keep seeding holdable PM-only gappers and watch the fade-tail ratio (3/15 = 20% after VIVK); Init 3's was to keep seeding held-name exits (+10%-limit edge robust at n=30). Both had fresh data today: one holdable footprint=none PM-only gapper logged 09-03 (GYGY +19.7%), and two held names exited at the 09-03 04:31 ET pulse (UFG $0.61, GELS $0.861).

**Step taken:** **Pilot / Initiative 6 (active slot):** re-ran `init6-pm-pilot.js` (35 holdable candidates now). **GYGY was skipped** by the continuation gate — its 04:00 spike faded below gap-mid then re-ramped at 04:40 through a thin middle (10-66k sh, 101-498 tr bars 04:05-04:25), failing the R+1/R+2 80%-of-high hold on real volume. Ledger `log/init6-pm-pilot.csv` unchanged in row count (15). **Parallel low-risk / Initiative 3:** pulled the two exit fills from `broker.js orders all --json` (UFG $0.61, GELS $0.861, both filled 08:31Z = 04:31 ET), measured each post-exit premarket SIP peak (capped 09:30 ET), appended both to `peak-seeking-exit-sim.js` (n=30 -> n=32) and `premarket-exit-gap.csv`, re-ran. All log-only, no orders.

**Result:** **Init 6:** no new entered row, but one existing row corrected. Last run VIVK 09-02 was recorded PM-last -9.5% (1-min bars incomplete when the script ran early); on the full 09-02 1-min SIP history VIVK's exit recomputes to flat (entry $1.05 = PM-last $1.05), reclassifying -9.5% -> 0.0%. Entered set stays **n=15** but SUM lifts +75.2% -> **+84.8% / mean +5.7% / median +10.0% / positive 12/15**, vs a PM-last floor of -6.8% and a do-nothing baseline of 0%. Net of ~2% spread ~+3.7%/name. The **fade-tail drops back to 2/15 (BIVI -27.1%, MIMI-2 -6.9%) = 13%** as VIVK leaves the tail — comfortably under the ~25% that erodes the edge below the spread. **Init 3:** GELS is a live-book spike (exit $0.861 -> PM peak $0.977 05:15 ET, 873k sh/2606 tr, **fills the resting +10% limit $0.9471**); UFG is a thin dead-book staller (exit $0.61 -> peak $0.6074 below exit, lim10 unfilled, PM-last -0.9%). Re-ran the sim: plain **+10% sell-limit stays the best rule at n=32: SUM +95.0% / mean +3.0%, positive 21/32** (up from n=30's +84.2/+2.8; GELS adds a +10 win, UFG a small drag).

**Hypothesis / next step:** Init 6 — the fade-tail is contained at 2/15 = 13% after VIVK corrected out; keep seeding holdable PM-only gappers and track the ratio. With n=15 / 12 positive / two converging studies the live PM-gapper pulse is near proposable, hold until n grows and the tail stays under ~25%. Init 3 — keep seeding held-name exits; the +10%-limit edge is robust at n=32 (positive 21/32) and dead-book stallers keep contributing ~0. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** two open asks unchanged (no new one). (1) The Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback; optional wide ~-15% catastrophe-stop). Evidence: +95.0% total / +3.0%/name over 32 seeds, positive 21/32. Veto window, not applied. (2) Still open from 08-07: the Initiative 2 broker/feed decision for the ~203% fill gap (Alpaca SIP ~$99/mo, IBKR paper, or modeled SIP fills).

---

### 2026-09-02 — Initiative 6 pilot: two fresh admits (JLHL limit-win, VIVK fade), entered n=15, edge holds +5.0%/name, fade-tail 3/15 (20%). Initiative 3 unblocked (KITT, PXS seeded, n=30)

**Evaluated:** The 2026-09-01 steps **worked.** Init 6's next step was to keep seeding holdable PM-only gappers and track the fade-tail ratio (2/13); Init 3's was to resume seeding held-name exits once a position exits. Both had fresh data today: two holdable footprint=none PM-only gappers logged 09-02 (JLHL +21.2%, VIVK +19.7%), and two held names exited at the 09-02 04:30 ET pulse (KITT $0.8702, PXS $5.98) — the first Init 3 seeds since 08-28.

**Step taken:** **Pilot / Initiative 6 (active slot):** re-ran `init6-pm-pilot.js` (34 holdable candidates now). **Both JLHL and VIVK were admitted** by the continuation gate — two fresh out-of-sample entries in one run. Ledger `log/init6-pm-pilot.csv` grows to 15 entered rows. **Parallel low-risk / Initiative 3:** pulled the two exit fills from `broker.js orders all --json`, measured each post-exit premarket SIP peak (capped 09:30 ET), appended both to `peak-seeking-exit-sim.js` (n=28 -> n=30) and `premarket-exit-gap.csv`, re-ran. All log-only, no orders.

**Result:** **Init 6:** JLHL (entered $6.81 at 04:45 ET) **filled the resting +10% limit ($7.49)** intrabar = a win at the exit rule; VIVK (entered $1.05 at 04:15 ET) never reached its +10% limit and drifted to PM-last -9.5% = a fade. Entered set moves n=13 -> **n=15: SUM +75.2% / mean +5.0% / median +10.0% / positive 12/15**, vs a PM-last hold-to-open floor of -7.5% and a do-nothing baseline of 0%. Net of ~2% spread ~+3.0%/name. The **fade-tail grows to 3/15 (BIVI -27.1%, MIMI-2 -6.9%, VIVK -9.5%) = 20%** — still under the ~25% that would erode the edge below the spread, but VIVK adds a third and the ratio is now worth watching closely. **Init 3:** both new seeds are dead-book stallers with the +10% limit unfilled and a small positive fallback — KITT exit $0.8702 -> PM peak $0.91 at 06:10 ET (+4.6%, 322k sh/957 tr, real liquid spike but under the +10% limit $0.957), PM-last +0.6%; PXS exit $5.98 -> PM peak $6.28 at 08:25 ET (+5.0%, thin 2-5 tr/bar), lim10 ($6.58) unfilled, PM-last +3.9%. Re-ran the sim: plain **+10% sell-limit stays the best rule at n=30: SUM +84.2% / mean +2.8%, positive 20/30** (edge unchanged from n=28's +76.1/+2.7; both new seeds positive, no drag).

**Hypothesis / next step:** Init 6 — the fade-tail ratio (now 3/15 = 20% after VIVK) is the key threat; if it crosses ~25% the mean edge erodes below the ~2% spread. Keep seeding holdable PM-only gappers and track the fade-tail ratio; with n=15 / 12 positive / two converging studies the live PM-gapper pulse is nearing proposable, hold until the tail confirms it stays contained. Init 3 — keep seeding held-name exits; the +10%-limit edge is robust at n=30 (positive 20/30) and dead-book stallers keep contributing small positives. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** two open asks unchanged (no new one). (1) The Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback; optional wide ~-15% catastrophe-stop). Evidence: +84.2% total / +2.8%/name over 30 seeds, positive 20/30. Veto window, not applied. (2) Still open from 08-07: the Initiative 2 broker/feed decision for the ~203% fill gap (Alpaca SIP ~$99/mo, IBKR paper, or modeled SIP fills).
...
**Step taken:** **Pilot / Initiative 6 (active slot):** re-ran `init6-pm-pilot.js` (27 holdable candidates now). **WHLR was admitted** by the continuation gate (entered $2.69 at 04:15 ET) — second admitted out-of-sample entry since the pilot started; NA was skipped (no qualifying ignition/hold). **Parallel low-risk / Initiative 3:** pulled the two exit fills from `broker.js orders all --json`, measured each post-exit premarket SIP peak (capped 09:30 ET), appended both to `peak-seeking-exit-sim.js` (n=26 -> n=28) and `premarket-exit-gap.csv`, re-ran. All log-only, no orders.

**Result:** **Init 6:** WHLR's early spike **filled the resting +10% limit** ($2.96) before it dumped to PM-last -45.4% — the exact peak-seeking case the resting limit is built for, and a WIN at the exit rule. Entered set moves n=11 -> **n=12: SUM +64.8% / mean +5.4% / median +10.0% / positive 10/12**, vs a PM-last floor of -9.1% and a do-nothing baseline of 0%. Net of ~2% spread ~+3.4%/name. The **fade-tail holds at 2/12 (BIVI -27.1%, MIMI -6.9% at lim10)** — not growing toward the ~1-in-4 that erodes the edge; last run's faded admit (MIMI) is now offset by a winning admit (WHLR). **Init 3:** both new seeds are dead-book stallers with the +10% limit unfilled — AEMD exit $3.17 -> peak $3.30 04:35 ET (+4.1%, 202k sh/1867 tr), lim10 ($3.49) unfilled, dumped to PM-last -6.0%; MODD exit $3.82 -> peak $4.19 07:25 ET (+9.7%, thin 10k sh/121 tr), lim10 ($4.20) unfilled by 1c, PM-last +3.4%. Re-ran the sim: plain **+10% sell-limit stays the robust best rule at n=28: SUM +76.1% / mean +2.7%, positive 18/28** (mean eased +3.0 -> +2.7 from the two dead-book seeds, one negative one slightly positive — small drag, conclusion unchanged; L15 sums higher but is PAVS/CELZ/GXAI-outlier-driven and much worse on the early-peak dumpers).

**Hypothesis / next step:** Init 6 — the fade-tail ratio (BIVI, MIMI) is the key threat; it held at 2/12 this run as WHLR filled the limit. Keep seeding holdable PM-only gappers and track the fade-tail ratio before proposing any live PM-gapper pulse. Init 3 — keep seeding held-name exits; the +10%-limit edge is robust at n=28 (positive 18/28) and dead-book stallers keep contributing ~0. Keep `CONFIRM-3` and all sims log-only.
...
**Step taken:** **Pilot / Initiative 6 (active slot):** re-ran `init6-pm-pilot.js` (24 holdable candidates now) — RDIB skipped for the same wick-high reason as WVVIP (04:30 ignition wicks $18.47, next bar closes $14.31 < 80% of the wick high, yet the name BUILDS to $17.44). Then built `scripts/init6-gate-denom-test.js` (log-only) to test the flagged fix: sweep the hold denominator (bar HIGH vs bar CLOSE) over all 43 footprint=none names, splitting admits by classification. **Parallel low-risk / Initiative 3:** today's 04:30-04:31 ET eval sold two held names (XPON $6.42, YYGH $2.02) — fresh out-of-sample seeds. Measured each post-exit premarket SIP peak, appended both to `peak-seeking-exit-sim.js` (n=22 -> n=24) and `premarket-exit-gap.csv`, re-ran. All log-only, no orders.

**Result:** **Init 6:** the close-denominator recovers RDIB + WXM within the holdable universe (entered would rise to n=12 / mean +6.8% / positive 11/12) but as a standalone gate also admits four uninvestable wick-fades — LICN -50%, DXST -15%, ZCMD/SFHG +10% — worsening the false-positive bucket from -16.3% to -61.3%. So the denominator relaxation only helps if paired with the discretionary holdable tag; it cannot replace the wick-high gate alone. WVVIP is not recovered by either denominator. **Decision: keep the conservative wick-high gate; entered set stays n=10 (SUM +61.7% / mean +6.2% / positive 9/10); log the wick-then-rebuild miss as a bounded, known cost.** **Init 3:** both new seeds are live-book names with a genuine post-exit premarket spike that fills the +10% limit — XPON $6.42 -> $7.15 05:10 ET (+11.4%), YYGH $2.02 -> $2.61 07:00 ET (+29.2%). Re-ran the sim: plain **+10% sell-limit strengthens to SUM +81.2% / mean +3.4%, positive 16 of 24** (up from +61.2% / +2.8% / 14/22 at n=22). The dumper tail (LOOP -20.5) is not growing (1 of the last 8 seeds).

**Hypothesis / next step:** Init 6 — the gate's wick-high denominator systematically rejects the biggest holdable BUILDs (WVVIP, RDIB) but a close-denominator over-admits wick-fades, so the fix is not free; keep the conservative gate and, if a third wick-then-rebuild holdable is missed, reconsider a holdable-gated close-denominator variant. Keep seeding holdable PM-only gappers and watch the BIVI-type fade-tail. Init 3 — keep seeding held names as they exit; the +10%-limit edge is robust at n=24. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** two open asks unchanged (no new one). (1) The Init 3 peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback; optional wide ~-15% catastrophe-stop). Evidence: +81.2% total / +3.4%/name over 24 seeds, positive 16/24. Veto window, not applied. (2) Still open from 08-07: the Initiative 2 broker/feed decision for the ~203% fill gap (Alpaca SIP ~$99/mo, IBKR paper, or modeled SIP fills).
...
**Step taken:** **Pilot / Initiative 3 (execution pivot):** pulled the three exit fills from `broker.js orders all --json` (SUGP $4.27, LGO $0.6701, ISPC $1.79, all filled 08:30:3xZ = 04:30 ET), measured each post-exit premarket peak from SIP 5-min bars (capped 09:30 ET), appended all three to `scripts/peak-seeking-exit-sim.js` (n=18 -> n=21) and `log/premarket-exit-gap.csv`. Then built the **OCO-floor variant** into the sim: resting +10% sell-limit paired with a protective stop F% below the exit price, evaluated causally per bar (stop checked first within a bar, pessimistic), swept F in {0,5,10,15,20}. `node --check` passes and the sim runs. Log-only, no orders.

**Result:** Plain **+10% limit stays the core rule at n=21: SUM +61.6% / mean +2.9%, positive 14/21** (the 3 dead-book seeds pulled the mean down from +3.7%). The OCO sweep **refutes the 08-20 breakeven-stop idea:** a stop AT the exit price (O0) collapses to SUM +20.8 / mean +1.0, positive only 3/21 — it whipsaws nearly every winner because the first post-exit bar dips through the exit price intrabar before the spike (PAVS/CELZ/BAOS/GXAI/XOS/MSS all stopped at 0% instead of +10%). A moderate floor (O5) still whipsaws dip-then-rip winners (BOXL +10->-5, MTEN +3->-5) and nets WORSE than plain L10 (+50.4 < +61.6). Only a **wide catastrophe-stop** helps at all: O15 (-15% floor) catches only the extreme dumper LOOP (-20.5 -> -15.0, +5.5 saved) with zero collateral whipsaw (SUM +67.1, 14/21); O10 marginally higher (+68.0) but whipsaws SUGP. The +5.5 improvement rests entirely on one dumper.

**Hypothesis / next step:** The core edge is unchanged (plain resting +10% sell-limit, 14/21 positive, +2.9%/name). The 08-20 "pair with a breakeven OCO stop" refinement is **rejected** — it destroys the edge via whipsaw. If any protective stop is added it must be a **wide ~-15% catastrophe-stop**, and even then the gain is minor and rests on the single LOOP tail. The proposal to Juan reverts to the clean version (resting +10% sell-limit, GTC premarket, cancel 09:30 ET, plain 04:30 market exit as fallback), with an optional wide catastrophe-stop noted but not required. Next run: keep seeding held names; watch dead-book-dumper frequency — LOOP/SUGP/ISPC are 3 of the last 5 seeds, so if the dumper tail keeps growing the wide catastrophe-stop becomes more valuable. Keep `CONFIRM-3` and both instruments log-only.

**Needs from Juan:** two open asks, one simplified. (1) The peak-seeking-exit proposal — replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit (GTC premarket, cancel 09:30 ET, plain market exit fallback); the 08-20 OCO-stop add-on is **withdrawn** (breakeven stop whipsaws the edge), optional wide ~-15% catastrophe-stop only. Veto window, not applied. Evidence: +61.6% total / +2.9%/name over 21 seeds, positive 14/21. (2) Still open from 08-07: the Initiative 2 broker/feed decision for the ~203% fill gap (Alpaca SIP ~$99/mo, IBKR paper, or modeled SIP fills).
...
**Step taken:** **Research / Initiative 6 (deferred test now unblocked):** the gate-admitted PM-only-gapper set reached **n=12** (FRGT 08-13 pushed it over the ~12 threshold the roadmap had been waiting for), so ran the long-deferred 1-minute exit test. Built `scripts/pm-gapper-exit-sim-1min.js` (log-only, no orders): it reuses the exact 5-min continuation gate to fix the admitted names + entry price, then walks 1-MINUTE SIP bars applying close-based trailing stops (8/12/15/20%) AND resting sell-limits at entry*(1+L%) for L in {10,15,20,30,50} that fill intrabar on the first 1-min bar whose high reaches the limit. `node --check` passes and it runs on all 12 admitted names.

**Result:** **First positive Init 6 result.** A resting **+10% sell-limit (lim10) captures mean +4.0% / median +10.0%, positive 9 of 12** — decisively beating every 5-min exit (all negative median, from the earlier sim) and every 1-min trailing stop (trail8 -0.2%, trail12 +0.9%/med -3.2%, all negative median). It fills intrabar 7/12; the only 3 losers are the non-holdable thin/uninvestable admits (WLDS -23%, BJDX -9%, EHGO-2 -3%), so on the 7 holdable admits lim10 is positive on all (~+9.8% mean). Root cause the 5-min sim missed: the peak prints intrabar (INLF entry bar $3.76 -> $7.19), so only a finer 1-min resting limit catches it. Higher limits (lim30/50) have higher mean but lower fill rate and are INLF-outlier-driven; lim10 is robust. **This converges with the 08-14 Init 3 peak-seeking finding — a resting +10% sell-limit was independently the winner there too (+6.1%/name, 8/9 positive).** Two separate studies (overnight AH holds; PM-only-gapper scalps) now point at the same exit mechanism.

**Hypothesis / next step:** A resting +10% sell-limit above entry is the right exit mechanism for both the PM-only-gapper scalp (Init 6) and the overnight AH hold (Init 3); the two independent +10%-limit results strengthen the single peak-seeking-exit proposal to Juan rather than adding a separate PM-gapper pulse. Next run: (Init 3 pilot) once DARE/GRSD/ONFO or any new held name exits, seed it into the peak-seeking sim to confirm the +10%-limit edge out-of-sample. (Init 6) keep growing the admitted set and consider tightening the gate to stop admitting thin/uninvestable names (WLDS/BJDX) that drag the mean. Keep `CONFIRM-3` and all sims log-only.

**Needs from Juan:** nothing new. Two open asks unchanged: (1) the firm peak-seeking-exit proposal from 08-14 (replace the plain 04:30 ET market exit for held names with a resting +10% sell-limit, GTC premarket, cancel 09:30 ET; now backed by a second converging study) — veto window, not applied; (2) still open from 08-07: the Initiative 2 broker/feed decision for the ~203% fill gap (Alpaca SIP ~$99/mo, IBKR paper, or modeled SIP fills).
