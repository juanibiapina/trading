...
**Latest status (2026-08-03): Third-bar verdict instrumented; no 5-minute pulse change justified.** The 2026-07-31 replay caught **3/3 winners and 0/6 fades**, including rejecting Juan's BOOM negative control, but confirmed 6.1% worse than the first current-grid price on average. Today added `scripts/ah-5m-confirmation.js` and wired its `CONFIRM-3` verdict into every >10% AH candidate scan as log-only data: it reports YES/NO/PENDING using the replay's local-volume ignition plus third-bar hold test and cannot affect entry, grading, ranking, or schedules. Replayed AMIX/NUWE/KUST as YES and ONMD/BOOM as NO. Next: collect out-of-sample candidates, including their next-PM outcome, before considering an entry gate. Full table and caveats are in `INIT3_IGNITION_TIMING.md`. Parallel checks: Init 6's admitted PM-gapper sample remains n=10, below the n>=12 trigger for its deferred one-minute exit test.
