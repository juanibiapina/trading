...
- [ ] Initiative 3 (execution pivot): **FIRMED PROPOSAL — replace the plain
      ...
      Evidence (08-14, `scripts/peak-seeking-exit-sim.js`, log-only, 9 seeds):
      the resting +10% sell-limit captures **+55.1% total / +6.1% per name,
      positive on 8 of 9** vs selling at 04:30, and is robust (+45.1% even
      excluding the PAVS outlier). It beats every trailing stop (best +25.8%,
      which whipsaws negative on several names) and beats the fixed second pulse
      the 08-13 sweep tested (+25.0% total, one-outlier-driven). The +10% limit
      wins because early peakers (BAOS, BOXL) spike then crash, so a modest
      resting limit fills into the first spike before the dump; a +15-20% limit
      never fills on those and falls to a deeply negative PM-last. Changes live
      exit behavior → **proposed, not applied**. Next run confirms the edge on
      1-2 more out-of-sample held names before any wiring. Details in the
      Initiative 3 status + `log/premarket-exit-gap.csv`.
