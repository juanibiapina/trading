...
- [ ] Initiative 3 (execution pivot): **SOFTENED PROPOSAL — a peak-seeking
      premarket exit (resting sell-limit / trailing stop set at entry), NOT a
      second fixed exit pulse.** Our single 10:30 CET (04:31 ET) position-eval
      sells Grade-None overnight holds early, and 9/9 seeded held names later
      spiked in premarket (peak-ceiling tally +251.1%). **But** the 08-13 sweep
      (`scripts/premarket-exit-pulse-sweep.js`, log-only) shows a *fixed* second
      pulse captures little of that: the best time (06:00 ET / 12:00 CET) nets
      only **+25.0% total over 9 names (~+2.8%/name), dominated by one outlier
      (PAVS)** — the early peakers (BAOS, BOXL, GXAI, WAFU) dump before any
      reasonable second pulse fires. So a blind second pulse is **not**
      supported. The better-evidenced ask is a peak-seeking exit that sells
      early spikes into strength. Changes live exit behavior → **proposed, not
      applied**; a future run first sims the resting-limit/trailing-stop capture
      over the 9 seeds. Details in the Initiative 3 status +
